	...
	$_SESSION[$final['pg_tag']]=array(
        ...
        switch($df_ip['ac']):
            case 'list': 
				...
				$final['assign_view']='item_list_index';
				$final['show_tcate']='Y';
				$final['show_plate']='Y';
				//$pd_pp=10;
				$pd_pp=$df_ip['pp'];
				...
				$ip_data=array('con_'.$mm_tb.'.isproductprep'=>'Y','con_'.$mm_tb.'.isactive'=>'Y','pd_pp'=>$pd_pp,'pd_np'=>$df_ip['np'],'gp'=>'Y','ob_'.$mm_tb.'.'.$df_ip['ob']=>$df_ip['ot'],'orwhere'=>array());
				...
				if($this->isadmin=='Y'):
					...
					array_push($ip_data['orwhere'],"((".$mm_tb.".jec_user_id IN (".$user_wi.") OR ".$mm_tb.".jec_usersales_id IN (".$user_wi."))) OR ".$mm_tb.".createdby='".$this->ad_id."'");
				...
				$final['tcate_url']=array(
						'create_item_index'=>site_url($final['var_purl'].'create_item_index/list/')
					);
            break;
			
            case 'delete_productprep':
               //
			   $prodp_set=$this->CM->Init_TB_Set('mm_productprep_set');
			   $this->$prodp_set['mm_set']->clean_project_prep($df_ip['key_id']);
			   //refresh.
                $ajo=array(
					'refresh_url'=>site_url($final['var_purl'].$df_ip['tag'].'/list/0/'.$final['var_surl']),
                    'pass'=>1
                );
