<?php $this->load->view('common/page_bar_div_f',array('fpd'=>$fpd)); ?>
	<table class="detail-div" cellspacing="1" cellpadding="3">
    	<tr>
    		<td colspan="4" class="mm_table2_title">追減附件列表</td>
    	</tr>
        <tr>
        	<td class="mm_table2_title2" <?=$fob=='created'?'id="'.$fpd['ob_css'].'"':'class="detail-nosort" id="detail-normal"'?>  onclick="ECP_Load_Div('file_area_div','<?=$fpd['ob_created_url']?>');">追減日期</td>
        ...
        <tr id="ftr_<?=$no?>">
            ...
            <td><input type="button" value="刪除附件" onclick="AWEA_Ajax('<?=site_url('ecp_common/delete_file/reload_file_list/'.$value['jec_projfile_id'].'/')?>','','del','ftr_<?=$no?>')"  class="mm_submit_button" <?=$this->ECPM->m_right_tag['del_da']?> ></td>    
        </tr>        
        <?php endforeach;?>
    </table>