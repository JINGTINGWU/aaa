		 ...
		 switch($type){
			 ...
			 case 'check_edit_1':
				//當待辦工作通知發出後。工作名稱、負責人員、督導人員、開始結束日期、通知天數、延遲天數，不可再修改
                                //160728:當工作屬於發包-管線機電,不得手動變更
                                $noAdjustTaskID=array(155,156,206,207);//20160728:行銷部曉莊需求:管線與機電發包不得手動變更
				$final=true;
				if($data['isnotice']=='Y'||$data['isfinish']=='Y'||in_array($data['jec_task_id'],$noAdjustTaskID)):
					$final=false;
				endif;
			 	break;
