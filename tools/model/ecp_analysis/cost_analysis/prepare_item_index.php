<?php
        switch($df_ip['ac']):
            case 'list': 
				...
				$final['list_url']=base_url($final['var_purl'].'prepare_item_index/list_div/'.$df_ip['key_id'].'/');
				$final['export_excel_url']=site_url($final['var_purl'].$df_ip['tag'].'/export_excel/'.$df_ip['key_id'].'/');
				$final['export_prep_excel_url']=site_url($final['var_purl'].$df_ip['tag'].'/export_prep_excel/'.$df_ip['key_id'].'/');

				$final['assign_view']='prepare_item_index';
				$final['show_tcate']='Y';
				$final['show_plate']='Y';
            ...
            break;
			case 'list_div':
				$final['assign_view']='prepare_item_div';
				$proj_v_set=$this->CM->Init_TB_Set('mm_project_search_set');
				$final['proj_data']=$this->$proj_v_set['mm_set']->get_project_row($df_ip['key_id']);
				//$final['main_list']=$this->project_init_full_view($df_ip['key_id']);
				$prodprep_v_set=$this->CM->Init_TB_Set('mm_productprep_search_set');
				$mm_tb=$prodprep_v_set['mm_tb'];
				//$ip_data=array('con_jec_project_id'=>$df_ip['key_id']);
				$ip_data=array('con_'.$mm_tb.'.jec_project_id'=>$df_ip['key_id'],'con_'.$mm_tb.'.isactive'=>'Y','ob_'.$mm_tb.'.seqno'=>'ASC');
				$final['main_list']=$this->GM->common_ac('list',array('info'=>$prodprep_v_set['mm_set'],'type'=>'join_projprod','data'=>$ip_data));
...
// Redirect output to a client’s web browser (Excel5)
			...
			case 'export_prep_excel':
				...
				*/
				...
				$mm_tb=$prodprep_v_set['mm_tb'];
				$ip_data=array('con_'.$mm_tb.'.jec_project_id'=>$df_ip['key_id'],'con_'.$mm_tb.'.isactive'=>'Y','ob_'.$mm_tb.'.seqno'=>'ASC');
				$main_list=$this->GM->common_ac('list',array('info'=>$prodprep_v_set['mm_set'],'type'=>'join_projprod','data'=>$ip_data));
				...
				$objPHPExcel->getProperties()->setCreator("EMS System")
							 ...
							 ->setTitle("履約備品清單分析")
							 ->setSubject("履約備品清單分析")
							 ->setDescription("履約備品清單分析")
							 ->setKeywords("履約備品清單分析")
							 ->setCategory("履約備品清單分析");
			    ...
			    $objPHPExcel->setActiveSheetIndex(0)
            				...
            				->setCellValue('B1', '品名')
            				->setCellValue('C1', '規格')
            				->setCellValue('D1', '預估數量')
							->setCellValue('E1', '預估單價')
							->setCellValue('F1', '預估成本')
							->setCellValue('G1', '實際數量')
							->setCellValue('H1', '實際單價')
							->setCellValue('I1', '減折讓')
							->setCellValue('J1', '實際成本')
							->setCellValue('K1', '業務成本')
							->setCellValue('L1', '預估-實際')
							->setCellValue('M1', '實際-業務')
							;
				// 
				
				$predict_final=0; $actual_final=0; $real_final=0; $real_quantity_final=0; $actual_quantity_final=0; $real_price_final=0; $actual_price_final=0; $totalsubtract_final=0;
                $eno=0; $e_task=0; foreach($main_list as $value): $eno++;
				    $real_total=$value['projprod_total']; $real_final+=$real_total; $predict_final+=$value['total']; $real_quantity_final+=$value['quantity'];
					$xno=$eno+1;
					if((int)$value['jec_projprod_id']>0):
						...
						$actual_total=number_format($actual_total);
						$actual_quantity_final+=$value['projprod_quantity'];//預估數量加總
					else:
						$actual_total='-';
					endif;						
			    	$objPHPExcel->setActiveSheetIndex(0)
            				->setCellValue('A'.$xno, $eno)
            				->setCellValue('B'.$xno, $value['prodname'])
            				->setCellValue('C'.$xno, $value['prodspec'])
            				->setCellValue('D'.$xno, (float)$value['quantity'])
							->setCellValue('E'.$xno, number_format((float)$value['price']))
							->setCellValue('F'.$xno, number_format((float)$value['total']))
							->setCellValue('G'.$xno, (float)$value['projprod_quantity'])
							->setCellValue('H'.$xno, number_format((float)$value['projprod_price']))	
							->setCellValue('I'.$xno, number_format((float)$value['projprod_totalsubtract']))						
							->setCellValue('J'.$xno, (int)$value['jec_projprod_id']>0?number_format((float)$real_total):'-')
							->setCellValue('K'.$xno, $actual_total)
							->setCellValue('L'.$xno, (int)$value['jec_projprod_id']>0?number_format($value['total']-(float)$real_total):'')
							->setCellValue('M'.$xno, (int)$value['jec_projprod_id']>0?number_format((float)$real_total-(float)$actual_total):'')
							;     
        			 endforeach;
					 $eno++;
					 $xno=$eno+1;
			    	...
			    	$objPHPExcel->setActiveSheetIndex(0)
            				...
            				->setCellValue('D'.$xno, '')
							->setCellValue('E'.$xno, '')
							->setCellValue('F'.$xno, number_format($predict_final))
							->setCellValue('G'.$xno, '')
							->setCellValue('H'.$xno, '')
							->setCellValue('I'.$xno, '')
							->setCellValue('J'.$xno, number_format($real_final))
							->setCellValue('K'.$xno, number_format($actual_final))
							->setCellValue('L'.$xno, number_format($predict_final-$real_final))
							->setCellValue('M'.$xno, number_format($real_final-$actual_final))
							;  
            
				// Rename sheet
				$objPHPExcel->getActiveSheet()->setTitle('履約備品清單分析');
...
// Redirect output to a client’s web browser (Excel5)
				header('Content-Type: application/vnd.ms-excel');
				header('Content-Disposition: attachment;filename="Prepare List-'.date('Y-m-d').'.'.$excel_type.'"');
				header('Cache-Control: max-age=0');
