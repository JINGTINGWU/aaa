<?php $this->load->view('common/page_bar_div',array('pd'=>$pd)); ?>
    ...
    {        
        if($('#recordtype').val()=='N')
        {            
            location.href='<?=base_url('/ecp_work/inform_item_mng/work_record_index/list/'.$key_id.'/created/DESC/0/-1.html')?>';
        }
        else
        {
            location.href='<?=base_url('/ecp_work/inform_item_mng/work_record_index/list-all/'.$key_id.'/created/DESC/0/-1.html')?>';
        }
    }
</script>