        ...
        switch($df_ip['ac']):
            case 'list': 
				...
				$final['tcate_url']=array(
						'project_init_index'=>base_url($final['var_purl'].'project_init_index/list/'.$final['projt_data']['jec_project_id'].'/created/asc/0/N/'),
			...
			case 'update_projprod':
				...
				$gv=array("projprod_id","description",'price','startdate','quantity','jec_vendor_id','no','extramultiple','extraaddition','totalsubtract','estimcostcalc','salecostcalc','prod_uom_id','prodspec','prodname'); $gv=$this->CM->GPV($gv);
				$no=$gv['no'];
				$projp_set=$this->CM->Init_TB_Set('mm_projprod_set');
				$upd=$gv;
