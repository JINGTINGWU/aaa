...
<tr><td>
        ...
        <div id="task_info_div" class="mm_area_div_3">
			<?php $this->load->view('common/task/task_info_div_t1',array('projt_data'=>$projt_data)); ?>
        </div>
</td></tr>
<tr><td colspan="2">
	<?php
$o_add_da=$this->ECPM->m_right_tag['add_da'];
if($proj_data['projstatus']==6) $o_add_da='disabled';
if($projt_data['isfinish'] == 'Y') $o_add_da='disabled';
			  if($o_add_da==''):
    ...
    <tr><td rowspan="2" class="info-title" width="60" align="center">新增料品<br /><br /><input type="button" value="新增料品" onclick="pg_submit('add_projprod','<?=$form_url?>');" class="mm_submit_button" <?=$add_da?> style="width:60px;" /><br /><input  class="mm_submit_button"  type="button" value="回上頁" onclick="JS_Link('<?=$tcate_url['bk_url']?>')"  style="width:60px;" /></td><td>
<div id="search_area_div"  class="mm_area_div_1">
    ...
    <?php
	$o_add_dp=$this->ECPM->m_right_tag['add_dp'];
	if($proj_data['projstatus']==6):
   		$o_add_dp='style="display:none;"';
	endif;
			  if($o_add_dp==''):
		  			$add_dp = $this->QIM->is_project_modify($proj_data['jec_project_id'])?'':'style="display:none;"';
		      else:
			  	    $add_dp = $o_add_dp;
			  endif;
	?>
	<table <?=$add_dp?> style="margin:0px;" >
		<tr>
        	<th width="100">選擇料品範本：</th><td><?=$main_op['import_producttemp']['op']?><br /><input type="button" value="從範本匯入料品清單" onclick="PG_BK_Action('import_producttemp',{})" style="width:130px;" <?=$add_da?> /></td>
            <th width="100">選擇EXCEL檔案：</th><td><?=$main_op['import_excel']['op']?></td><td><input type="button" value="從EXCEL匯入料品清單" onclick="pg_submit('import_prod_excel');" <?=$add_da?> />
            
            <?php if($proj_data['isproductprep']=='Y'):?>
           		<?php if($proj_data['isworkflow']=='Y'):?>
           	 	<input type="button" value="從履約備品清單匯入料品清單" onclick="pg_submit('import_prod_prepare');" <?=$add_da?> />
                <?php else:?>
                <input type="button" value="本專案的履約備品清單尚未批核完成" disabled="disabled" />
            	<?php endif;?>
            
            <?php else:?>
            	<input type="button" value="本專案尚未建立履約備品清單" disabled="disabled" />
            <?php endif;?>
    ...
    <div id="add_jobdetail_div">
		...
		<?php $this->load->view($this->m_controller.'/create_project/add_jobdetail_div',array('main_op'=>$main_op,'form_url'=>$form_url,'proj_data'=>$proj_data)); ?>
...
<div id="result_area_div"  class="mm_area_div_2">
	<?php $this->load->view($this->m_controller.'/create_project/jobdetail_list_div',array('main_list'=>$main_list,'uom_pdb'=>$uom_pdb,'pd'=>$pd,'proj_data'=>$proj_data)); ?>
</div> 
</td></tr>
...
{	var info=Array();	
	switch(type){
		...
		case 'import_prod_excel':
			...
			break;
		case 'import_prod_prepare':
			thick_box('open','備品清單列表','<?=base_url('ecp_common/select_prepare_item/'.$proj_data['jec_project_id'].'/')?>?width=850&height=520');
...
{
	switch(type){
		case 'import_selected_item':
			data.projtask_id='<?=$key_id?>';
			//alert(data.select_string+"="+data.project_id+"="+data.projtask_id);
			//alert('<?=$import_selected_url?>');
			AWEA_Ajax('<?=$import_selected_url?>',data,'');
			break;
		case 'after_import_selected':
			thick_box('close');
		...
		case 'update_projprod'://
			...
			data.totalsubtract=$("#totalsubtract_"+data.no).val();
		...
		case 'recount_row_cost':
			...
			var base_total=0;	
			if(parseFloat(cost)==0){	//抓預估的
				base_total=accMul(qty,unit_price);
			}else{	//抓實際的 
				base_total=accMul(qty,cost);
			}
			base_total=accMul(base_total,multi_var)+add_var;	
			$("#salecostcalc_tag_"+data).html(base_total+'');
			/*
			var base_total=parseFloat($("#total_tag_"+data).html());
		...
		case 'change_prod_pq'://change_tag_
			...
			fi_nod(document.getElementById("totalsubtract_"+data));
			var unit_price=$("#price_"+data).val();
			var subtract=$("#totalsubtract_"+data).val();
			if(unit_price==''){ 
				$("#price_"+data).val(0);
				unit_price=0;
			}
		...
		case 'prod_search_prodtype':
			...
			if(InStr(old_url,'/product_only')){
				var target_tag='/product_only';
			}
			if(target_tag==''){
				var target_tag='/product';
			}			
			...
			switch(data){
				case 'work':
					...
					break;
				case 'prod':
					new_url=old_url.replace(target_tag,'/product_only');
					break;
				case 'full':
...
var InputList={
		...
		info:{ 
				...
				?>
				vendor:{ search_list:'pl_search_list',search_value:'search_value',search_here:'jec_vendor_id_title',search_div:'pl_search_div',url:'<?=base_url('ecp_common/search_select/vendor/')?>','input_id':'jec_vendor_id','input_type':'R','width':250,'title_id':'jec_vendor_id_title' }
