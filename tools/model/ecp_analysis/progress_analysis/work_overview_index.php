                ...
                $eno=0; $e_task=0; foreach($main_list as $st1=>$sv1): $eno++;
			    	...
			    	$objPHPExcel->setActiveSheetIndex(0)
        	              ...
        	              foreach($sv1['data'] as $st2=>$sv2): $e_task++; $e_task_v=0; $e_prod_f=0; $eno++; 
            				...
            				->setCellValue('C'.$xno, '')
            				->setCellValue('D'.$xno, $sv2['row']['taskname'])
							->setCellValue('E'.$xno, $sv2['row']['sales_name'])
							->setCellValue('F'.$xno, substr($sv2['row']['startdate'],0,10).'~'.substr($sv2['row']['enddate'],0,10))
							->setCellValue('G'.$xno, $sv2['row']['isfinish'])
							->setCellValue('H'.$xno, $sv2['row']['isfinish']=='Y'?substr($sv2['row']['finishdate'],0,10):'')
							->setCellValue('I'.$xno, '')
							->setCellValue('J'.$xno, '')
							->setCellValue('K'.$xno, '')
							->setCellValue('L'.$xno, '')
