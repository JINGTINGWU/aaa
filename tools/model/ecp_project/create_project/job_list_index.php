...
//$pd_pp=10;	
        ...
        switch($df_ip['ac']):
            case 'list': 
				...
				$final['form_url']=site_url($final['var_purl'].'job_list_index/add_task/'.$df_ip['key_id'].'/');
				$final['add_task_url']=site_url($final['var_purl'].'job_list_index/add_task_div/'.$df_ip['key_id'].'/');
				$final['task_list_url']=site_url($final['var_purl'].'job_list_index/list_div/'.$df_ip['key_id'].'/created/asc/0/N/');
				...
				$final['update_projtask_url']=base_url($final['var_purl'].'job_list_index/update_projtask/'.$df_ip['key_id'].'/');
				$final['after_select_task_url']=base_url($final['var_purl'].$df_ip['tag'].'/after_select_task/'.$df_ip['key_id'].'/');
                ...
                $final['main_data']=array('taskdaynotice'=>1,'taskdaydelay'=>0,'taskprocesstype'=>2,'taskconfirmtype'=>1,'taskworkweight'=>1); 
				...
				$final['tcate_url']=array(
						...
						'mission_list_index'=>base_url($final['var_purl'].'mission_list_index/list/'.$final['projj_data']['jec_project_id'].'/created/asc/0/N/'),
						'bk_url'=>base_url($final['var_purl'].'mission_list_index/list/'.$final['projj_data']['jec_project_id'].'/created/asc/0/N/')
					);//
            break;
			...
			case 'add_task_div':
				$final['form_url']=site_url($final['var_purl'].'job_list_index/add_task/'.$df_ip['key_id'].'/');
				$final['add_task_url']=site_url($final['var_purl'].'job_list_index/add_task_div/'.$df_ip['key_id'].'/');
				$final['task_list_url']=site_url($final['var_purl'].'job_list_index/list_div/'.$df_ip['key_id'].'/');
				$projj_v_set=$this->CM->Init_TB_Set('mm_projjob_search_set');
				$final['projj_data']=$this->GM->common_ac('list',array('info'=>$projj_v_set['mm_set'],'upt'=>'def','kid'=>$df_ip['key_id']));
				...
				$final['ip_info']['jec_user_id']['full_selected']='Y';
				$final['ip_info']['description']['style']='width:70%';
                $final['main_data']=array('taskdaynotice'=>1,'taskdaydelay'=>0,'taskprocesstype'=>2,'taskconfirmtype'=>1,'taskworkweight'=>1); //修改新增後的預設值為強制
				//$final['ip_info']['jec_task_id']['ip_dl_exclude']=$this->CM->FormatData(array('db'=>$final['main_list'],'key'=>'jec_task_id'),'page_db','s_array');
				$final['ip_info']['jec_task_id_title']['onchange']="PG_BK_Action('change_taskname',{})";
				$final['main_op']=$this->form_input->each_op_trans('full',$final['ip_info'],$final['main_data']);
				$proj_set=$this->CM->Init_TB_Set('mm_project_set');
			...
			case 'update_projtask'://
				...
				//$ori_status=$this->GM->GetSpecData($projt_set['mm_tb'],'projtasktype',$projt_set['mm_kid'],$gv['projtask_id']);
