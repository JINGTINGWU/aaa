...
&nbsp; <?=$main_op['excel_type']['op']?><input type="button" value="明細資料匯出Excel" class="mm_submit_button"  onclick="pg_submit('export_excel');" />-->
        ...
        <tr>
            ...
            <td class="mm_table2_title2" <?=$ob=='price'?'id="'.$pd['ob_css'].'"':'class="detail-nosort" id="detail-normal"'?>><a onclick="ECP_Load_Div('result_area_div','<?=$pd['ob_price_url']?>');">預估單價</a></td> 
        ...
        <tr id="tr_<?=$no?>">
            ...
            <td ><?=$e_op['oppro_value']['op']?></td>
            <td ><?=$e_op['prodname']['op']?></td>
            <td ><?=$e_op['prodspec']['op']?></td>  
            ...
            <td ><input type="button" value="修改"  onclick="PG_BK_Action('update_projprod',{'projprod_id':'<?=$value['jec_productprep_id']?>','no':'<?=$no?>'})" class="mm_submit_button_1"  <?=$up_da?> ></td> 
