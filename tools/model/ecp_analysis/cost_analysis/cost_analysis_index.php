        ...
        switch($df_ip['ac']):
            case 'list': 
				...
				$final['tcate_url']=array( 
						...
						'prepare_item_index'=>base_url($final['var_purl'].'prepare_item_index/list/'.$df_ip['key_id'].'/')
					);
            break;
			case 'list_div':
			...
			case 'export_excel':
			    ...
			    $objPHPExcel->setActiveSheetIndex(0)
							...
							->setCellValue('E1', '實際成本')
							->setCellValue('F1', '業務成本')
							->setCellValue('G1', '預估-實際')
							->setCellValue('H1', '實際-業務')
							;

				// Miscellaneous glyphs, UTF-8
				
				$full_assum_cost=0; $full_sale_cost=0; $full_real_cost=0; $now_job=0; $now_task=0; $eno=0; $e_task=0;
				foreach($final['main_list'] as $st1=>$sv1): $eno++;
					...
					foreach($sv1['data'] as $st2=>$sv2): $e_task++; $e_task_v=0; $e_prod_f=0; $eno++; $now_task=$eno;
						...
						$task_xno=$xno;
						 ...
						 $e_count_prod_assum=0; $e_count_prod_cost=0; $e_count_prod_real=0; foreach($sv2['data'] as $sv3): //$eno++;
						 	//$xno=$eno+2;
				 	...
				 	//$e_task_v=$sv2['row']['isfinish']=='Y'?'100':'0'; 
						 ...
						 ->setCellValue('E'.$task_xno, number_format($e_task_real))
						 ->setCellValue('F'.$task_xno, number_format($e_task_cost))
						 ->setCellValue('G'.$task_xno, number_format($e_task_deff))
						 ->setCellValue('H'.$task_xno, number_format($e_task_deff2))
						 ;
					...
					endforeach;
					 ...
					 $full_real_cost+=$e_job_real;
						 ...
						 ->setCellValue('E'.$job_xno, number_format($e_job_real))
						 ->setCellValue('F'.$job_xno, number_format($e_job_cost))
						 ->setCellValue('G'.$job_xno, number_format($e_job_assum-$e_job_real))
						 ->setCellValue('H'.$job_xno, number_format($e_job_real-$e_job_cost))
						 ;					 
				endforeach;
				$xno++;
            			 ...
            			 ->setCellValue('A'.$xno, "合計")
           				 ->setCellValue('B'.$xno,"")
						 ->setCellValue('C'.$xno, "成本合計")
						 ->setCellValue('D'.$xno, number_format($full_assum_cost))
						 ->setCellValue('E'.$xno, number_format($full_real_cost))
						 ->setCellValue('F'.$xno, number_format($full_sale_cost))
						 ->setCellValue('G'.$xno, number_format($full_assum_cost-$full_real_cost))
						 ->setCellValue('H'.$xno, number_format($full_real_cost-$full_sale_cost))
						 ;
				/*		 
				$xno++;
				...
				$multi_sale_cost=$full_sale_cost*(float)$proj_data['costrate'];
            			 ...
            			 ->setCellValue('A'.$xno, "")
           				 ->setCellValue('B'.$xno,"")
						 ->setCellValue('C'.$xno, "業務成本係數比率=".(float)$proj_data['costrate'])
						 ->setCellValue('D'.$xno, '')
						 ->setCellValue('E'.$xno, '')
						 ->setCellValue('F'.$xno, '')
						 ->setCellValue('G'.$xno, number_format($multi_sale_cost))
						 ->setCellValue('H'.$xno, '')
						 ; 
			    ...
			    $xno++;
            			 ...
            			 ->setCellValue('A'.$xno, "")
           				 ->setCellValue('B'.$xno,"")
						 ->setCellValue('C'.$xno, "實際-業務成本")
						 ->setCellValue('D'.$xno, '')
						 ->setCellValue('E'.$xno, '')
						 ->setCellValue('F'.$xno, '')
						 ->setCellValue('G'.$xno, number_format(($full_real_cost-$multi_sale_cost)))
						 ->setCellValue('H'.$xno, '')
				     	 ; 
