        ...
        switch($df_ip['ac']):
            case 'list': 
				...
				$final['tcate_url']=array(
						...
						'confirm_list_index'=>base_url($final['var_purl'].'confirm_list_index/list/'.$df_ip['key_id'],'/')
					);
				$final['btn_allow']=$this->$projn_set['mm_set']->btn_allow;
				if(!in_array($final['projn_data']['noticetype'],$final['btn_allow'])) unset($final['tcate_url']['confirm_list_index']);
            break;
			case 'list-all': 
				if($df_ip['chinfo']==''):
					$_SESSION[$final['pg_tag']]['np']=$df_ip['np'];
				...
				$projr_v_set=$this->CM->Init_TB_Set('mm_projrecord_search_set');
				$final['main_list']=$this->GM->common_ac('list',array('info'=>$projr_v_set['mm_set'],'type'=>'def','data'=>array('con_jec_projtask_id'=>$final['projn_data']['jec_projtask_id'],'con_isactive'=>'Y','ob_'.$df_ip['ob']=>$df_ip['ot'],'pd_np'=>$df_ip['np'],'pd_pp'=>$pd_pp)));
                $_G['pg_div_function']='ECP_Load_Div';
                $_G['pg_div_load']='Y';
                $_G['pg_div_type']='';
                $_G['pg_div_id']='result_area_div';
			...
			case 'list_div-all':
				...
				$final['assign_view']='work_record_div';
				$projr_v_set=$this->CM->Init_TB_Set('mm_projrecord_search_set');
				$final['main_list']=$this->GM->common_ac('list',array('info'=>$projr_v_set['mm_set'],'type'=>'def','data'=>array('con_jec_projtask_id'=>$final['projn_data']['jec_projtask_id'],'con_isactive'=>'Y','gp'=>'Y','ob_'.$df_ip['ob']=>$df_ip['ot'],'pd_np'=>$df_ip['np'],'pd_pp'=>$pd_pp)));
				$_G['pg_div_function']='ECP_Load_Div'; //
                $_G['pg_div_load']='Y';
                $_G['pg_div_type']='';
                $_G['pg_div_id']='result_area_div'; //
