...
//$pd_pp=10;
        ...
        switch($df_ip['ac']):
            case 'list': 
				...
				$final['file_list_url']=site_url($final['var_purl'].'job_list_index/file_list_div/'.$df_ip['key_id'].'/created/asc/0/N/');
			...
			case 'target': 
				...
				$projj_v_set=$this->CM->Init_TB_Set('mm_projjob_search_set');
				$mm_tb=$projj_v_set['mm_tb'];
				$ip_data=array('con_'.$mm_tb.'.jec_project_id'=>$df_ip['key_id'],'con_'.$mm_tb.'.isactive'=>'Y','gp'=>'Y','pd_pp'=>$pd_pp,'pd_np'=>$df_ip['np'],'ob_'.$mm_tb.'.'.$df_ip['ob']=>$df_ip['ot'],'con_'.$mm_tb.'.jec_projjob_id'=>$df_ip['task_id']);
				if($this->isadmin=='Y'):
					$type='def';
				else:
					$type='join_task';
			...
			case 'add_job':
				...
				foreach($append_task as $value)://$value
					...
					$upd=array(
							...
							'description'=>$e_task['description'],
