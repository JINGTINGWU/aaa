...
class Common_model extends CI_Model {
    ...
    var $title   = '';
	var $sys_email ='';  // 通知信件寄件人電子郵件
	var $sys_name = ''; // 通知信件寄件人名稱
	var $sys_server = ''; // 郵件主機
	var $sys_port = ''; // 郵件主機PORT
	var $sys_account = ''; // 郵件主機帳號
	var $sys_password = ''; // 郵件主機密碼
    
    function __construct()
    {   global $_G;
        parent::__construct();
        
		$sender_email = $this->db->where('noticetype','EM')->where('isactive','Y')->get('jec_setup')->result_array();
		$sender_email = $sender_email[0]['value'];
		$this->sys_email = $sender_email;
		
		$get_name = $this->db->where('noticetype','EN')->where('isactive','Y')->get('jec_setup')->result_array();
		$get_name = $get_name[0]['value'];
		$this->sys_name = $get_name;
		
		$get_server = $this->db->where('noticetype','ES')->where('isactive','Y')->get('jec_setup')->result_array();
		$get_server = $get_server[0]['value'];
		$this->sys_server = $get_server;
		
		$get_port = $this->db->where('noticetype','ET')->where('isactive','Y')->get('jec_setup')->result_array();
		$get_port = $get_port[0]['value'];
		$this->sys_port = $get_port;
		
		$get_account = $this->db->where('noticetype','EU')->where('isactive','Y')->get('jec_setup')->result_array();
		$get_account = $get_account[0]['value'];
		$this->sys_account = $get_account;
		
		$get_password = $this->db->where('noticetype','EV')->where('isactive','Y')->get('jec_setup')->result_array();
		$get_password = $get_password[0]['value'];
		$this->sys_password = $get_password;
    } 
	/*
	...
	*/
    ...
    function JS_Msg($msg='',$code=0,$type='p')
    {
		if($type=='p'){
			?><script>
            	parent.ECP_Msg('<?php echo $msg;?>','<?php echo $code;?>');
            </script><?php
		}else{
			?><script>
            	ECP_Msg('<?php echo $msg;?>','<?php echo $code;?>');
            </script><?php
		}
		if($type=='error'){
			?><script>
            	parent.ECP_Msg('<?php echo $msg;?>','<?php echo $code;?>','error');
            </script><?php
		}else{
			?><script>
            	ECP_Msg('<?php echo $msg;?>','<?php echo $code;?>','error');
            </script><?php
		}
		/*
    ...
    function SendMailGo($receive_email,$receive_name,$title,$content,$sender_email,$sender_name){
		/*
		if(@mail($receive_email,iconv('utf-8','big5',$title),iconv('utf-8','big5',$content)."\n","MIME-Version: 1.0\nFrom: $sender_email\nReply-To: $sender_email\nErrors-To: $sender_email\nContent-Type:text/html; charset=big5\n\n")):
			return true;
		else:
			return false;
		endif;*/
		...
		$mail->Host = $this->sys_server;
		//$mail->Port = 25; //Gamil的SMTP主機的SMTP埠位為465埠
		$mail->Port = $this->sys_port;
		
		//$mail->Username = "pms"; //設定驗證帳號
		$mail->Username = $this->sys_account;
		//$mail->Password = "pms_5050207"; //設定驗證密碼
		$mail->Password = $this->sys_password;
        ...
        switch($type){
            ...
            case "number":
			    switch($num){
					...
					case 'ch_num':
					//$num=$value; $mode=true;
					$num=$value['num']; $mode=isset($value['mode'])?$value['mode']:true;
					
					
	$char = array("零","壹","貳","參","肆","伍","陸","柒","捌","玖");
	...
	if($ar[2][0] != "")
		$retval = $dec . $this->FormatData(array('num'=>$ar[2][0],'mode'=>false),'number','ch_num'); //如果有小數，先遞歸處理小數
	if($ar[1][0] != "")
	{
		$str = strrev($ar[1][0]);
            ...
            case "page_db":
                switch($num){
                    case 1:  //value->key,vf,db
						...
						if(isset($value['add_zero'])&&$value['add_zero']=='Y'&&!isset($final[0])) $final[0]='';
                    break;
                    case 'order'://排序
                        $final=array();
                        $vf=$value['vf'];
                    ...
                    case 'num'://sn/en/len/key/value
						$final=array();
						array_push($final,array($value['key']=>0,$value['value']=>""));
					...
					case 'mssql_ld':
						//check_is_trans -
						if(isset($value['istrans'])):
							$istrans=$value['istrans'];
						else:
							$os=$this->db->where('noticetype','OS')->where('isactive','Y')->get('jec_setup')->result_array();
							$os=strtolower($os[0]['value']);
							$istrans=$os=='linux'?'Y':'N';
						endif;
						$istrans='N';//wait to change
	...
	function from_db_trans($value='',$type='input')
	{	global $_G;
		/*
		if($_G['os']=='LINUX'):
			switch($type):
	...
	function pdb_trans($value='')
	{	global $_G;
		//$value=iconv('utf-8','big5',$value);
		return $value;
	}	
	function ReadFileName($type='read',$value='')
	{	$final = $value;
		switch($type):
			case 'save':
				//$final=iconv('utf-8','big5',$value);
				break;
			case 'read':
				//$final=mb_convert_encoding($value, "big5", "auto");
				//$final=iconv('utf-8','big5',$value);
				break;
			case 'download':
				//$final=iconv('utf-8','big5',$value);
