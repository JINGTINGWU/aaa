...
class Get_model extends CI_Model {
	...
	*/	
    ...
    function get_preaction($tb='',$data=array())
	{
		...
		$add=array('where'=>'where','dis'=>'dis','con'=>'con','wni'=>'wni','wi'=>'wi','jsql'=>'jsql','ob'=>'orderby','gp'=>'gp','pd'=>'page_data','join'=>'join','like'=>'like','row_type'=>'row_type','one_field'=>'one_field','non'=>'non','orwhere'=>'orwhere','fixselect'=>'fixselect','group_by'=>'group_by');//可能為外加的絛件
	...
	{   global $_G;
		...
		if(isset($data['page_data']['pp'])&&isset($data['page_data']['np']))
		{
       ...
       // echo $config['total_rows']."-".$config['per_page']."-".$config['cur_page']."-".$config['base_url']."-";
        ...
        {   //排序用的，輸入替換位置與對應欄位
			...
			if(isset($config['now_ot'])):
				if($config['now_ot']=='ASC'):
					$r_full_url=substr($full_url,0,strlen($full_url)-6).'DESC/0/';
					$d_full_url=$full_url;
				else:
					$r_full_url=substr($full_url,0,strlen($full_url)-7).'ASC/0/';
					$d_full_url=substr($full_url,0,strlen($full_url)-7).'ASC/0/';
				endif;
			else:
				$r_full_url=$full_url;
				$d_full_url=$full_url;
