...
<div id="result_area_div" class="mm_area_div_2" onclick="PL_DivClick();">
	...
	<table class="detail-div" cellspacing="1" cellpadding="3">
    	<tr>
    		<td colspan="10" class="mm_table2_title">已立案專案列表(專案狀態：得標前準備)</td>
    	</tr>
        <tr>
            ...
            <td class="mm_table2_title2" id="detail-normal">得標/工作開展</td>
        ...
        <tr>
            ...
            <td><?=$jtp_data['j']?>/<?=$jtp_data['t']?>/<?=$jtp_data['p']?></td> 
            <td><input type="button" value="得標/工作開展" onclick="JS_Link('<?=site_url($var_purl.'project_init_index/list/'.$value['jec_project_id'].'/')?>');" class="mm_submit_button"  /></td>
            <td><input type="button" value="上傳附件" onclick="thick_box('open','檔案上傳','<?=site_url('ecp_common/file_upload_div/project_list/'.$value['jec_project_id'].'/')?>')" class="mm_submit_button" <?=$this->ECPM->m_right_tag['up_da']?> /></td>
        </tr>        
        <?php endforeach;?>
    </table>
</div>
