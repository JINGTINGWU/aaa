...
if($df_ip['key_id']<0||!isset($_SESSION[$final['pg_tag']])){
	...
	switch($df_ip['chinfo']):
		case 'Unconfirm':
			...
			$_SESSION[$final['pg_tag']]['s_string']='4,5,6,7,8,9';
			//4,5,6,8,9
			break;
		case 'Alert':
	...
	$df_ip['chinfo']='N';
        ...
        switch($df_ip['ac']):
            case 'list': 
				...
				$final['pd']=$this->GM->page_data(array('now_ot'=>$df_ip['ot'],'uri_segment'=>$df_ip['seg'],'base_url'=>base_url().$this->m_controller.'/'.$this->m_function.'/'.$df_ip['tag'].'/'.$df_ip['ac'].'/'.$df_ip['key_id'].'/'.$df_ip['ob'].'/'.$df_ip['ot'].'-'.$pd_pp.'/','mm_spec_url'=>array('ot_desc_url'=>'DESC@@@'.$df_ip['ot'],'ot_asc_url'=>'ASC@@@'.$df_ip['ot'],'ob_proj_name_url'=>'proj_name@@@'.$df_ip['ob'],'ob_isconfirm_url'=>'isconfirm@@@'.$df_ip['ob'],'ob_task_name_url'=>'task_name@@@'.$df_ip['ob'],'ob_noticetype_url'=>'noticetype@@@'.$df_ip['ob'],'ob_emailcontent_url'=>'emailcontent@@@'.$df_ip['ob'],'ob_noticetime_url'=>'noticetime@@@'.$df_ip['ob'])));
