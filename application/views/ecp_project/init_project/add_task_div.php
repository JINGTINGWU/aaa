...
?>
	<table class="query-div" cellspacing="1" cellpadding="3"  style="margin-top:-6px;">
		...
		<tr>
        	<td rowspan="3" width="70"><input type="button" value="新增工作" onclick="pg_submit('add_task','<?=$form_url?>');" class="mm_submit_button" <?=$add_da?>  style="width:70px;" /><input  class="mm_submit_button"  type="button" value="回上頁" onclick="JS_Link('<?=$tcate_url['bk_url']?>')"  style="width:70px;" /></td>        	<td width="104" align="right">*選擇工作項目：</td>
       	  <td  id="frominputarea" colspan="3"><input type="checkbox" id="tasksearch_type" onclick="PG_BK_Action('task_search_type','tasksearch_type')"  />只查我的部門<?=$main_op['jec_task_id_title']['op']?><?=$main_op['jec_task_id']['op']?><br /><label id="jec_task_id_title_lb"></label></td>
		...
		<tr>
            ...
            <td align="right">備註說明：</td><td colspan="4" id="frominputarea"><?=$main_op['description']['op']?><input type="button" value="片語輸入" onclick="thick_box('open','片語輸入','<?=site_url('ecp_common/phrase_search_div/description/')?>')"  class="mm_submit_button" <?=$add_da?>  style="width:70px;" /></td>
  </tr>
	</table>
<script>
