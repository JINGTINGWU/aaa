...
{   //場地管理->MMMM
    ...
    function room($df_ip=array())  //說明
    {   global $_G;
        $final['field_pre']=$this->field_pre;
        $final['response_type']='normal';
        $mm_set='mm_arearoom_set';
        $up_f=array("name","mintable","maxtable","tablenote","pricelist","areameasure","height","seqno","floor");
        $final['ccate_ptag']='room';
        $final['ccate_menu']=array(
            ...
            'room_act_project'=>array(
                    'title'=>'適用專案',
                    'index'=>'list'
                ),
            'room_add_project'=>array(
                    'title'=>'加入專案',
                    'index'=>'list'
                ),
            'button_bk'=>array(
        ...
        include $this->m_lib_folder."room.php";
        

        //echo $df_ip['ac']."-".$df_ip['pg'];
        return $final;        
    }
    function room_pic($df_ip=array())  //
    {   global $_G;
        $this->CM->close_cache();
        $final['field_pre']=$this->field_pre;
        $final['response_type']='normal';
        ...
        $up_f=array("isopen","name","description","ad_image_id");//
        $p_data=explode("_",$df_ip['chinfo']);
        $final['room_id']=isset($p_data[1])?$p_data[1]:0;  
        //$var_surl=$df_ip['ob']."/".$df_ip['ot']."/".$df_ip['np']."/"; 
        //$var_purl=$this->m_controller."/".$df_ip['s_cate']."_mng/"; 
        $final=$this->CM->get_df_url($final,$df_ip);
        include $this->m_lib_folder."room_pic.php";      

        //echo $df_ip['ac']."-".$df_ip['pg'];
        return $final;        
    }
    function room_planpic($df_ip=array())  //
    {   global $_G;
        $this->CM->close_cache();
        $final['field_pre']=$this->field_pre;
        $final['response_type']='normal';
        ...
        //$var_purl=$this->m_controller."/".$df_ip['s_cate']."_mng/"; 
        $final=$this->CM->get_df_url($final,$df_ip);
        include $this->m_lib_folder."room_planpic.php";      

        //echo $df_ip['ac']."-".$df_ip['pg'];
        return $final;        
    }
    
    function room_act_project($df_ip=array())  //
    {   global $_G;
        $final['field_pre']=$this->field_pre;
        $final['response_type']='normal';
        ...
        $p_data=explode("_",$df_ip['chinfo']);
        $final['arearoom_id']=isset($p_data[1])?$p_data[1]:0; 
        $final=$this->CM->get_df_url($final,$df_ip);
        include $this->m_lib_folder."room_act_project.php";   
