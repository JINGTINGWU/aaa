...
class Ecp_system extends CI_Controller 
{
    var $mm_base_url='';
    var $m_controller='ecp_system';
    var $m_pp=10; //統一的每頁筆數  
    ...
    function index($tag='projtask_trans_index',$ac='list',$key_id=0,$ob='created',$ot='DESC',$np=0,$chinfo='')
    {   global $_G; $MDST=$this->def_info['MDST']; $pp=$this->m_pp; $seg=7; $s_cate='projtask_trans';
    ...
    function projtask_trans_mng($tag='projtask_trans_index',$ac='list',$key_id=0,$ob='created',$ot='DESC',$np=0,$chinfo='')
    {   global $_G; $MDST=$this->def_info['MDST']; $pp=$this->m_pp; $seg=7; $s_cate='projtask_trans';
        if(isset($_POST['kid'])) $key_id=$_POST['kid'];
        $this->load->model('Mmm_'.$s_cate.'_model',$MDST,True);
        $df_array=$this->df_array;
