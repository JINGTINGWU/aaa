...
//$pd_pp=5;
        ...
        switch($df_ip['ac']):
            case 'list': 
				...
				$final['assign_view']='project_invoice_index';
				$final['show_tcate']='Y';
				$final['show_plate']='Y';
				$proji_set=$this->CM->Init_TB_Set('mm_projinvoice_set');//34
				$final['main_list']=$this->GM->common_ac('list',array('info'=>$proji_set['mm_set'],'type'=>'def','data'=>array('con_jec_project_id'=>$df_ip['key_id'],'gp'=>'Y','pd_pp'=>$pd_pp,'pd_np'=>$df_ip['np'],'ob_'.$df_ip['ob']=>$df_ip['ot'])));
                ...
                $final['main_data']=array('invoicedate'=>date('Y-m-d'),'invoiceyear'=>date('Y')); 
				...
				$final['tcate_url']=array(
						...
						'project_cost_index'=>base_url($final['var_purl'].'project_cost_index/list/'.$df_ip['key_id'].'/created/asc/0/N/')
					);
            break;
			case 'list_div':
				if($df_ip['chinfo']==''):
