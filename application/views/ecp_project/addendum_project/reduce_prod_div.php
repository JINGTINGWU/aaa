	<table class="query-div" cellspacing="1" cellpadding="3" <?=$this->ECPM->m_right_tag['add_dp']?> >
		...
		<tr>
        	<td>追減日期：</td><td id="frominputarea"><?=$main_op['addsubdate']['op']?></td>
            <td>追減類型：</td><td id="frominputarea"><?=$main_op['addsubtype']['op']?></td>
            <td>追減料品：</td><td id="frominputarea"><?=$main_op['jec_product_id_title']['op']?><?=$main_op['jec_product_id']['op']?><br /><label id="jec_product_id_title_lb"></label></td>
        </tr>
		<tr>
        	<td>追減數量</td><td id="frominputarea"><?=$main_op['quantity']['op']?></td>
            <td>預估單價</td><td id="frominputarea"><?=$main_op['price']['op']?></td>
        	<td>備註說明：</td><td id="frominputarea"><?=$main_op['description']['op']?><input type="button" value="片語輸入" onclick="thick_box('open','片語輸入','<?=site_url('ecp_common/phrase_search_div/description/')?>')"  class="mm_submit_button" /><input type="button" value="追減料品" onclick="pg_submit('add_prod','<?=$form_url?>');" class="mm_submit_button" /></td>
        </tr>
	</table>
