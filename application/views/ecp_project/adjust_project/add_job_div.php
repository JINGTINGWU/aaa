...
?>
	<table class="query-div" cellspacing="1" cellpadding="3" >
		...
		<tr>
        	<td align="right">*任務名稱：</td><td id="frominputarea"><input type="checkbox" id="jobsearch_type" onclick="PG_BK_Action('job_search_type','jobsearch_type')" checked="checked" />只查我的部門<?=$main_op['jec_job_id_title']['op']?><?=$main_op['jec_job_id']['op']?><br /><label id="jec_job_id_title_lb"></label></td><td><input type="button" value="新增任務" onclick="pg_submit('add_job','<?=$form_url?>');" class="mm_submit_button" <?=$add_da?> /></td></tr>
            <tr>
