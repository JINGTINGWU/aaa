...
<div id="add_task_div"  class="mm_area_div_1" onclick="PL_DivClick();">
	<?php $this->load->view($this->m_controller.'/adjust_project/add_task_div',array('main_op'=>$main_op,'form_url'=>$form_url,'proj_data'=>$proj_data)); ?>
</div>
<div id="result_area_div"  class="mm_area_div_2">
	<?php $this->load->view($this->m_controller.'/adjust_project/task_list_div',array('main_list'=>$main_list,'pd'=>$pd,'user_ld'=>$ip_info['jec_user_id']['ld'],'proj_data'=>$proj_data)); ?>
</div>
<div id="file_area_div" class="mm_area_div_2">
	<?php $this->load->view($this->m_controller.'/adjust_project/job_file_list_div',array('main_list'=>$file_list,'fpd'=>$fpd,'proj_data'=>$proj_data)); ?>
</div>
        </td>
    </tr>
</table>
...
{	
	...
	info[1]={sd_id:'jec_user_id_title',ed_id:'jec_usersuper_id_title',msg:'負責人與督導人不得相同',type:'quote'};
	if($("#startdate").val()!='' && $("#enddate").val()!=''){
		info[2]={sd_id:'startdate',ed_id:'enddate',msg:'結束日不能小於起始日',type:'sed'};
	}	
	//if($("#startdate").val()!='' && $("#jec_user_id_title").val()=='')
	//{				
	//	info[2]={id:'jec_user_id_title',msg:'指定日期後負責人不可空白',type:'ne'};
	//}
	msg=fi_submit_check(info);
	if(msg==''){
		if($("#jec_user_id_title").val()!='' || $("#jec_usersuper_id_title").val()!=''){			
			data={ bk_action:'update_task_go',sales_name:$("#jec_user_id_title").val(),super_name:$("#jec_usersuper_id_title").val()};
...
{	var info=Array();  var msg='';		
	switch(type){
		...
		case 'update_task_go':
			if(GetTagV(data,'isexist')=='Y'){
				mvalue={
						...
						jec_usersuper_id_title:$("#jec_usersuper_id_title").val(),
						price:$("#price").val(),
		...
		case 'update_projtask':
			...
			if(data.taskname=="")
			{
				data.taskname=$("#taskname_"+data.no).text();
			}
			data.no=data.no;
			...
			if(data.taskname.indexOf('售服') >=0 && data.taskname.indexOf('施工')>=0)
				info[3]={sd_id:'enddate_'+data.no,ed_id:'proj_limitdate',msg:'售服施工結束日不能晚於合約結束日',type:'sed'};
			if(data.taskname==""){
				info[3]={id:'taskname_'+data.no,msg:'工作名稱不可空白',type:'ne'};
			}	
			if(data.taskname.indexOf('完工') >=0)
				info[3]={sd_id:'enddate_'+data.no,ed_id:'proj_limitdate_pre30',msg:'完工結束日不能晚於合約結束日前1個月',type:'sed'};		
			//日期填入後,人員不可空白			
			//if($("#startdate_"+data.no).val()!='0000-00-00' && $("#jec_user_id_title_"+data.no).val()=='')
			//{				
			//	info[3]={id:'jec_user_id_title_'+data.no,msg:'指定日期後負責人不可空白',type:'ne'};
			//}
...
<input type="hidden" id="proj_limitdate" value="<?=$proj_data['limitdate']?>" />
<input type="hidden" id="proj_limitdate_pre30" value="<?=date("Y-m-d", strtotime($proj_data['limitdate']."-30 day"))?>" />
<script>
