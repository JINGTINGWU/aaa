	...
	$_SESSION[$final['pg_tag']]=array(
        ...
        switch($df_ip['ac']):
            case 'list': 
				...
				$sql="SELECT a.jec_project_id,a.description FROM jec_projnotice as a inner join jec_project as b on a.jec_project_id=b.jec_project_id inner join jec_dept as c on c.jec_dept_id=b.jec_dept_id WHERE noticetype IN(4,5,6,7,8,9,10,31) AND c.jec_user_id='".$this->ad_id."' AND a.isactive='Y' and a.description in ('批次展期','批次刪除') group by a.jec_project_id";				
				$projdata=$this->db->query($sql)->result_array();
				$allprojid=array();
				$allprojdesc=array();
				...
				{
					array_push($allprojid,$row['jec_project_id']);
					$allprojdesc[$row['jec_project_id']]=$row['description'];
   				}
				}
				else
				{
					array_push($allprojid,'');
				}
				$final['allprojdesc']=$allprojdesc;
				$ip_data=array('con_'.$mm_tb.'.isactive'=>'Y','pd_pp'=>$pd_pp,'pd_np'=>$df_ip['np'],'gp'=>'Y','ob_'.$mm_tb.'.'.$df_ip['ob']=>$df_ip['ot'],'wi'=>array('jec_project_search_view.jec_project_id'=>$allprojid),'orwhere'=>array());
				...
				if($_SESSION[$final['pg_tag']]['s_proj_projtype']!='') $ip_data['con_'.$mm_tb.'.projtype']=$_SESSION[$final['pg_tag']]['s_proj_projtype'];
				...
				if($this->isadmin=='Y'):
					...
					array_push($ip_data['orwhere'],"jec_projtask.jec_user_id IN (".$user_wi.") OR jec_projtask.jec_usersuper_id IN (".$user_wi.") OR ".$mm_tb.".jec_user_id IN (".$user_wi.") OR ".$mm_tb.".jec_usersales_id IN (".$user_wi.") OR jec_projtask.jec_group_id IN (".$group_wi.") OR ".$mm_tb.".createdby='".$this->ad_id."'");
