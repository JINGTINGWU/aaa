...
?>
	<table class="query-div" cellspacing="1" cellpadding="3" >
		...
		<tr>
            ...
            <td id="frominputarea"><?=$main_op['value']['op']?><?=$main_op['jec_product_id']['op']?></td><label id="value_lb"></label>
        	<td align="right" width="100">*品名：</td><td id="frominputarea" colspan="3"><input type="checkbox" id="prodsearch_type" onclick="PG_BK_Action('prod_search_type','prodsearch_type')" />只查我的部門<?=$main_op['jec_product_id_title']['op']?><?=$main_op['jec_product_id']['op']?><br /><label id="jec_product_id_title_lb"></label></td>
        ...
        <tr>
            ...
            <td align="right">預估單價(未稅)：</td><td id="frominputarea"><?=$main_op['price']['op']?></td>   
            <!-- <td align="right">預估合計：</td><td id="frominputarea" id="predict_amount"></td> -->
		...
		<tr>
            ...
            <td align="right">備註說明：</td><td colspan="4" id="frominputarea"><?=$main_op['description']['op']?><input type="button" value="片語輸入" onclick="thick_box('open','片語輸入','<?=site_url('ecp_common/phrase_search_div/description/')?>')"  class="mm_submit_button" <?=$add_da?> /></td>
        </tr>
	</table>
    <!--
