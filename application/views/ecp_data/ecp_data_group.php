...
<div id="container">
		...
		<div class="clearfix">
			<div class="tab-div-left">
				<ul id="functiontabs">
					...
					<li><a href=# onclick="loadme(1)" class="selected">人員群組</a></li>
	...
	<div id="user_group">
		<table class="query-div" cellspacing="1" cellpadding="3" style="margin-top: 10px;">
			<tr>
				<td colspan="2" align="left">新增群組名稱</td>
			</tr>
			<tr>
			...
			<tr>
				<td colspan="2" align="center">
					<?php if ($authority['isinsert']=='Y'): ?>
						<input type="button" id="btn_new" value="確定新增群組名稱" onclick="data_new()" />
					<?php else: ?>
						---
					<?php endif; ?>
				</td>
		...
		<table class="detail-div" cellspacing="1" cellpadding="3" style="margin-top: 10px;">
			<tr>
				<td colspan="4">現有群組名稱列表</td>
			</tr>
			<tr>
				<td id="detail-normal">群組名稱</td>
				<td id="detail-normal">備註說明</td>
				<td align="center" id="detail-normal">修改</td>
				<td align="center" id="detail-normal">刪除</td>
			</tr>
			<?php foreach($group_list as $key => $grouplist): ?>
				<tr>
					<td>
						<input type="text" id="<?= 'name'.$grouplist['jec_group_id'] ?>" 
						value="<?= $grouplist['name'] ?>" size="38" />
					</td>
					<td>
						<input type="text" id="<?= 'description'.$grouplist['jec_group_id'] ?>" 
						value="<?= $grouplist['description'] ?>" size="88" />
					</td>
					<td align="center">
						<?php if ($authority['isupdate']=='Y'): ?>
							<input type="button" id="<?= 'btn_update'.$grouplist['jec_group_id'] ?>" 
							value="修改" onclick="data_update(<?= $grouplist['jec_group_id'] ?>)" />
						<?php else: ?>
							---
						<?php endif; ?>
					</td>
					<td align="center">
						<?php if ($authority['isdelete']=='Y'): ?>
							<input type="button" id="<?= 'btn_delete'.$grouplist['jec_group_id'] ?>" 
							value="刪除" onclick="data_delete(<?= $grouplist['jec_group_id'] ?>)" />
						<?php else: ?>
							---
						<?php endif; ?>
					</td>
		...
		<script type="text/javascript">
			...
			function data_new()
			{
				if ($.trim($("#name").val()) == "")
				{
					humane.error("群組名稱不可空白");
					return;
				}
				//if (! confirm("是否確認要新增群組名稱?")) return;
				para_data = {
					name: $("#name").val(),
				...
				$.ajax({
					type: "POST",
					url: "<?=site_url('ecp_data_title/group_new/')?>",
					data: (para_data),
					success: function(result){
						if (result)
						{
							humane.info('群組名稱新增完成');
							pagereload();
						}
						else
							humane.error('群組名稱新增作業失敗！');
					}
				});
			}
			// 修改
			function data_update(id)
			{
				...
				$.ajax({
					type: "POST",
					url: "<?=site_url('ecp_data_title/group_update/')?>",
					data: (para_data),
					success: function(result){
						if (result)
							humane.info('群組名稱更新完成');
						else
							humane.error('群組名稱更新作業失敗！');
					}
				});
			}
			// 刪除
			...
			{
				if (! confirm("是否確認要刪除群組名稱?")) return;
				para_data = {
					jec_group_id: id
				};
				$.ajax({
					type: "POST",
					url: "<?=site_url('ecp_data_title/group_delete/')?>",
					data: (para_data),
					success: function(result){
						if (result)
						{
							humane.info('群組名稱刪除完成');
							pagereload();
						}
						else
							humane.error('群組名稱刪除作業失敗！');
					}
				});
			}
		</script>
