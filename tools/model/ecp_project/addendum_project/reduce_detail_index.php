...
//$pd_pp=5;
        ...
        switch($df_ip['ac']):
            case 'list': 
				...
				$final['assign_view']='reduce_detail_list';
				$final['show_tcate']='Y';
				$final['show_plate']='Y';
				$projas_v_set=$this->CM->Init_TB_Set('mm_projaddsub_search_set');//34
				$final['main_list']=$this->GM->common_ac('list',array('info'=>$projas_v_set['mm_set'],'type'=>'def','data'=>array('con_jec_project_id'=>$df_ip['key_id'],'wi'=>array('addsubtype'=>array(3,4)),'gp'=>'Y','pd_pp'=>$pd_pp,'pd_np'=>$df_ip['np'],'ob_'.$df_ip['ob']=>$df_ip['ot'])));
				...
				unset($final['ip_info']['addsubtype']['ld'][0]);
				unset($final['ip_info']['addsubtype']['ld'][1]);
				...
				$final['file_list']=$this->GM->common_ac('list',array('info'=>$projf_v_set['mm_set'],'type'=>'def','data'=>array('con_jec_project_id'=>$df_ip['key_id'],'con_filetype'=>4,'gp'=>'Y','pd_pp'=>$_SESSION[$final['pg_tag']]['fpp'],'pd_np'=>$_SESSION[$final['pg_tag']]['fnp'],'ob_'.$_SESSION[$final['pg_tag']]['fob']=>$_SESSION[$final['pg_tag']]['fot'])));
				...
				$final['tcate_url']=array(
						...
						'add_detail_index'=>base_url($final['var_purl'].'add_detail_index/list/'.$df_ip['key_id'].'/created/asc/0/N/')
					);
            break;
			case 'list_div':
				if($df_ip['chinfo']==''):
				...
				$pd_pp=$df_ip['pp'];
				$final['assign_view']='r_prod_list_div';
				$this->load->library('form_input');
			...
			case 'file_list_div':
				...
				$pd_pp=$df_ip['pp'];
				$final['assign_view']='r_file_list_div';
				$projf_v_set=$this->CM->Init_TB_Set('mm_projfile_search_set');
				$final['file_list']=$this->GM->common_ac('list',array('info'=>$projf_v_set['mm_set'],'type'=>'def','data'=>array('con_jec_project_id'=>$df_ip['key_id'],'con_filetype'=>4,'gp'=>'Y','pd_pp'=>$pd_pp,'pd_np'=>$df_ip['np'],'ob_'.$df_ip['ob']=>$df_ip['ot'])));
                $_G['pg_div_function']='ECP_Load_Div';
                $_G['pg_div_load']='Y';
                $_G['pg_div_type']='';
				$_G['pg_div_id']='file_area_div'; 
			...
			case 'add_prod_div':
				...
				$final['assign_view']='reduce_prod_div';
				$projas_v_set=$this->CM->Init_TB_Set('mm_projaddsub_search_set');
				$this->load->library('form_input');
                //$final['field_pre']=$this->field_pre;
                $final['ip_info']=$this->$projas_v_set['mm_set']->load_mm_field_check();
