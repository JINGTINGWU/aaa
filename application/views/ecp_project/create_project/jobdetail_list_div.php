<?php 
//$up_right=$this->QIM->menu_right_check($this->ECPM->m_menu_right,array('update'))?'':'disabled';
...
<?php $this->load->view('common/page_bar_div',array('pd'=>$pd)); ?>
	<table class="detail-div" cellspacing="1" cellpadding="3">
    	<tr>
    		<td colspan="16" class="mm_table2_title"><!--<?=$pd['view_type']=='prod'?'class="mm_table2_title"':'onclick="ECP_Load_Div(\'result_area_div\',\''.$pd['vt_prod_url'].'\');"'?><?=$pd['view_type']=='work'?'class="mm_table2_title"':'onclick="ECP_Load_Div(\'result_area_div\',\''.$pd['vt_work_url'].'\');"'?>-->料品清單<a ></a>與工作明細<a ></a>列表 
...
&nbsp; <?=$main_op['excel_type']['op']?><input type="button" value="明細資料匯出Excel" class="mm_submit_button"  onclick="pg_submit('export_excel');" />
        ...
        <tr>
            ...
            <td class="mm_table2_title2" width="100" <?=$ob=='name'?'id="'.$pd['ob_css'].'"':'class="detail-nosort" id="detail-normal"'?>><a  onclick="ECP_Load_Div('result_area_div','<?=$pd['ob_name_url']?>');">品名</a></td>
           ...
           <!-- <td class="mm_table2_title2" <?=$ob=='estimcostcalc'?'id="'.$pd['ob_css'].'"':'class="detail-nosort" id="detail-normal"'?>><a onclick="ECP_Load_Div('result_area_div','<?=$pd['ob_estimcostcalc_url']?>');">預估成本</a></td>-->
        ...
        <tr id="tr_<?=$no?>">
            ...
            <img src="<?=base_url()?>images/up.gif"  onclick="AWEA_Ajax('<?=site_url($var_purl.'job_detail_index/move_up/'.$value['jec_projprod_id'].'/'.$var_surl)?>','')" />&nbsp;<img src="<?=base_url()?>images/down.gif" onclick="AWEA_Ajax('<?=site_url($var_purl.'job_detail_index/move_down/'.$value['jec_projprod_id'].'/'.$var_surl)?>','')" />
            <?php endif;?>
            </td>
            <!--<td <?=$value['prodtype']==9?$mm_work_style:''?>><?=(int)$value['jec_productopen_id']>0?$e_op['oppro_value']['op']:$value['value']?></td>-->
            <td <?=$value['prodtype']==9?$mm_work_style:''?>><?=$isUserKey=='Y'?$e_op['prodname']['op']:$value['prodname']?></td>
            <td <?=$value['prodtype']==9?$mm_work_style:''?>><?=$isUserKey=='Y'?$e_op['prodspec']['op']:$value['prodspec']?></td>  
            <td <?=$value['prodtype']==9?$mm_work_style:''?>><?=$isUserKey=='Y'?$e_op['prod_uom_id']['op']:$uom_pdb[(int)$value['prod_uom_id']]?></td>   
            ...
            <td id="total_tag_<?=$no?>" <?=$value['prodtype']==9?$mm_work_style:''?>><?=(float)$value['total']?></td> 
			...
			<td <?=$value['prodtype']==9?$mm_work_style:''?> id="jec_vendor_id_title_lb_<?=$no?>"><?=$e_op['jec_vendor_id']['op']?><?=$e_op['jec_vendor_id_title']['op']?></td> 
            <td <?=$value['prodtype']==9?$mm_work_style:''?>><?=$e_op['description']['op']?><!--<?=$value['description']?>--></td> 
            <td <?=$value['prodtype']==9?$mm_work_style:''?>><?php if($value['isexport']=='N'):?>
            ...
            <input type="button" value="刪除" onclick="AWEA_Ajax('<?=site_url($var_purl.'job_detail_index/delete_projprod/'.$value['jec_projprod_id'].'/'.$var_surl)?>','','del','tr_<?=$no?>')" class="mm_submit_button_1"  <?=$del_da?>  >
            	<?php endif;?>
            </td>  
            <script>
