...
class Mm_calendar_search_set
{
	...
	{   global $_G;
	    $this->mm_td=array(
		        'join_projtask'=>array(   //noticetype
						'join'=>array('type'=>'left','jtb'=>'jec_projtask','jkey'=>'jec_projtask_id','mkey'=>'jec_projtask_id','tb'=>$this->mm_tb),
                        'con'=>array($this->mm_tb.'.isactive'=>'Y'),
						'jsql'=>array($this->mm_tb.'.*','jec_projtask.projtasktype'),
						'glt'=>4		
				    ),																	
				'def'=>array()
		    );
	...
	{	$final=array();
		...
		foreach($data as $value)://
			/*
			if((int)$value['jec_projtask_id']==0):
				echo '----'.$value['task_name'].'=='.$value['name'].'---<br>';
			endif;*/
			...
			for($i=0;$i<$e_days;$i++):
				...
				$task_startdate=substr($value['startdate'],0,10);
				$task_enddate=substr($value['enddate'],0,10);
				//+
				$value['css']="mm_mid_projtask";
				if($e_day==$task_startdate) $value['css']="mm_start_projtask";
				if($e_day==$task_enddate) $value['css']="mm_end_projtask";
				if($value['isfinish']=='Y') $value['css']="mm_done_projtask";
