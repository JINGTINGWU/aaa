<?php $this->load->view('common/page_bar_div',array('pd'=>$pd)); ?>
	<table class="detail-div" cellspacing="1" cellpadding="3">
    	<tr>
    		<td colspan="13" class="mm_table2_title">專案追加明細列表</td>
    	</tr>
        <tr>
        	<td class="mm_table2_title2" <?=$ob=='addsubdate'?'id="'.$pd['ob_css'].'"':'class="detail-nosort" id="detail-normal"'?>  onclick="ECP_Load_Div('result_area_div','<?=$pd['ob_addsubdate_url']?>');">追加日期</td>
            <td class="mm_table2_title2" <?=$ob=='addsubtype'?'id="'.$pd['ob_css'].'"':'class="detail-nosort" id="detail-normal"'?>  onclick="ECP_Load_Div('result_area_div','<?=$pd['ob_addsubtype_url']?>');">追加類型</td>
        ...
        <tr id="tr_<?=$no?>">
            ...
            <td><input type="button" value="上傳" onclick="thick_box('open','檔案上傳','<?=site_url('ecp_common/file_upload_div/add_prod_list/'.$value['jec_project_id'].'/reload_file_list/')?>')"  class="mm_submit_button_1" <?=$this->ECPM->m_right_tag['up_da']?> ></td>
