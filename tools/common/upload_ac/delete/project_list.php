...
$save_folder='';
		foreach($saved_info as $value):
			if(!in_array($value,$keep_info)):
				$this->CM->db->where($projf_set['mm_kid'],$saved_key[$value])->update($projf_set['mm_tb'],array('isactive'=>'N'));
				
			else:
				//copy
				//get_file_name
				$efile_data=$this->CM->db->where($projf_set['mm_kid'],$saved_key[$value])->get($projf_set['mm_tb'])->result_array();
