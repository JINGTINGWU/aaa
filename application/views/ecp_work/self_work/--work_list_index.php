...
<div id="search_area_div"  class="mm_area_div_1" onclick="PL_DivClick();">
	<table class="query-div" cellspacing="1" cellpadding="3" >
		...
		<tr>
        	<td align="right">專案負責：</td><td id="frominputarea"><?=$s_main_op['s_task_proj_user_id_title']['op']?><?=$s_main_op['s_task_proj_user_id']['op']?></td>
            <td align="right">任務名稱：</td><td id="frominputarea"><?=$s_main_op['s_job_name']['op']?></td>
            <td>關鍵字：</td>
            <td id="frominputarea"><?=$s_main_op['s_task_keyword']['op']?></td>
        </tr>
		<tr>
		  <td align="right">工程(採購)編號</td><td id="frominputarea"><?=$s_main_op['s_proj_value' ]['op']?></td>		
		  <td colspan="6" align="right"><input type="button" value="查詢工作資料" onclick="PG_BK_Action('search_go')" class="mm_submit_button" /></td>
	  </tr>
	</table>
</div>
<div id="result_area_div" class="mm_area_div_2"  onclick="PL_DivClick();">
	...
	<table class="detail-div" cellspacing="1" cellpadding="3">
    	<tr>
    		<td colspan="16">工作項目列表 (說明: 明細按鈕呈現黃色者, 表示已有明細在內)</td>
    	</tr>
        <tr>
			...
			<td class="mm_table2_title2"  id="detail-normal" >專案編號</td>
          ...
          <td class="mm_table2_title2" <?=$ob=='job_name'?'id="'.$pd['ob_css'].'"':'class="detail-nosort" id="detail-normal"'?>><a href="<?=$pd['ob_job_name_url']?>">任務名稱</a></td> 
            ...
            <td class="mm_table2_title2" <?=$ob=='enddate'?'id="'.$pd['ob_css'].'"':'class="detail-nosort" id="detail-normal"'?>><a href="<?=$pd['ob_enddate_url']?>" >工作日期</a></td> 
	...
	<td style="width:70px;word-break: break-all;"><a href='<?=$this->config->item('base_url')?>ecp_project/adjust_project_mng/project_adjust_index/list/<?=$value['jec_project_id']?>.html'><?=$value['value2']?></a>
			
			<td style="width:70px;word-break: break-all;"><?=$value['proj_value']?></td>						
				 
			
			
            <td style="width:150px;word-break: break-all;"><a href='<?=$this->config->item('base_url')?>ecp_project/adjust_project_mng/project_adjust_index/list/<?=$value['jec_project_id']?>.html'><?=$value['proj_name']?></a><?=$value['isproductprep']=='Y'?' || <a href="'.$this->config->item('base_url').'ecp_project/prepare_item_mng/item_submit_index/list/'.$value['jec_project_id'].'/created/asc/0/N">備品清單</a> ':''?></td>
			
          <td style="width:150px;word-break: break-all;"><a href='<?=$this->config->item('base_url')?>
		  ecp_project/adjust_project_mng/job_list_index/list/<?=$value['jec_project_id']?>/created/asc/0/-1'><?=$value['job_name']?></a></td> 
            <td style="width:150px;word-break: break-all;"><a href='<?=$this->config->item('base_url')?>ecp_project/adjust_project_mng/task_list_index/target/<?=$value['jec_projjob_id']?>/created/asc/0/-1.html/<?=$value['jec_projtask_id']?>'><?=$value['taskname']?></a></td>
            <td style="width:150px;word-break: break-all;"><?=$value['description']?></td>
            ...
            <td style="width:80px;word-break: break-all;"><?=$value['confirmtime']=='0000-00-00 00:00:00'?'':substr($value['confirmtime'],0,10)?>
            </td> 
            <td style="width:50px;"><?=$value['super_name']?></td>
            <td style="width:40px;" align="center"><input type="button" value="明細" onclick="JS_Link('<?=site_url($var_purl.'work_detail_index/list/'.$value['jec_projtask_id'].'/')?>');" class="mm_submit_button" style="width:40px;<?=$value['isprojprod']=='Y'?'background-color:#FFE97F;':''?>" ></td>
            <td style="width:40px;" align="center"><?php if(($value['projtasktype']==2&&$value['replystatus']==0)||($value['projtasktype']==5&&$value['replystatus']==0)):?><input type="button" value="回報" onclick="JS_Link('<?=site_url($var_purl.'work_report_index/list/'.$value['jec_projtask_id'].'/')?>');" class="mm_submit_button"  ><?php elseif(($value['projtasktype']==2&&$value['replystatus']!=0)||($value['projtasktype']==5&&$value['replystatus']!=0)): ?><input type="button" value="取消&#13;&#10;回報" onclick="JS_Link('<?=site_url($var_purl.'work_report_index/list/'.$value['jec_projtask_id'].'/')?>');" class="mm_submit_button"><?php endif;?></td></td>  
            <td style="width:40px;" align="center"><input type="button" value="紀錄" onclick="JS_Link('<?=site_url($var_purl.'work_record_index/list/'.$value['jec_projtask_id'].'/created/DESC/0/-1/')?>');" class="mm_submit_button"></td>  
