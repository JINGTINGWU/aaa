...
<div id="result_area_div" class="mm_area_div_2">
	...
	<table class="detail-div" cellspacing="1" cellpadding="3">
        ...
        <tr>
            ...
            <td class="mm_table2_title2" <?=$ob=='as_estimated_cost'?'id="'.$pd['ob_css'].'"':'class="detail-nosort" id="detail-normal"'?>><a href="<?=$pd['ob_as_estimated_cost_url']?>" >預估成本</a></td> 
            <td class="mm_table2_title2" <?=$ob=='add_estimated_cost'?'id="'.$pd['ob_css'].'"':'class="detail-nosort" id="detail-normal"'?>><a href="<?=$pd['ob_add_estimated_cost_url']?>" >追加成本</a></td>
            <td class="mm_table2_title2" <?=$ob=='sub_estimated_cost'?'id="'.$pd['ob_css'].'"':'class="detail-nosort" id="detail-normal"'?>><a href="<?=$pd['ob_sub_estimated_cost_url']?>" >追減成本</a></td>  
            <!--<td>成本合計</td> --> 
            <td  id="detail-normal">追加明細</td>  
            <td  id="detail-normal">追減明細</td> 
        </tr>
        ...
        <tr>
            ...
            <td><?=number_format($value['as_estimated_cost'])?></td> 
            <td><?=number_format($value['add_estimated_cost'])?></td>
            <td><?=number_format($value['sub_estimated_cost'])?></td>  
            <!--<td>成本合計</td>-->  
            <td><input type="button" value="追加明細" onclick="JS_Link('<?=site_url($var_purl.'add_detail_index/list/'.$value['jec_project_id'].'/seqno/DESC/0/-1/')?>');" ></td>  
            <td><input type="button" value="追減明細" onclick="JS_Link('<?=site_url($var_purl.'reduce_detail_index/list/'.$value['jec_project_id'].'/seqno/DESC/0/-1/')?>');" ></td> 
        </tr>        
        <?php endforeach;?>
    </table>
</div>
