...
<div id="container">
		...
		<div class="clearfix">
			<div class="tab-div-left">
				<ul id="functiontabs">
					...
					<li><a href=# onclick="loadme(2)" class="selected">費用名稱</a></li>
	...
	<div id="charge_item">
		<table class="query-div" cellspacing="1" cellpadding="3" style="margin-top: 10px;">
			<tr>
				<td colspan="2" align="left">新增費用名稱</td>
			</tr>
			<tr>
				<td align="right">費用名稱：</td>
				<td id="frominputarea">
			...
			<tr>
				<td colspan="2" align="center">
					<?php if ($authority['isinsert']=='Y'): ?>
						<input type="button" id="btn_new" value="確定新增費用名稱" onclick="data_new()" />
					<?php else: ?>
						---
					<?php endif; ?>
				</td>
		...
		<table class="detail-div" cellspacing="1" cellpadding="3" style="margin-top: 10px;">
			<tr>
				<td colspan="4">現有費用名稱列表</td>
			</tr>
			<tr>
				<td id="detail-normal">費用名稱</td>
				<td id="detail-normal">備註說明</td>
				<td align="center" id="detail-normal">修改</td>
				<td align="center" id="detail-normal">刪除</td>
			</tr>
			<?php foreach($charge_list as $key => $chargelist): ?>
				<tr>
					<td>
						<input type="text" id="<?= 'name'.$chargelist['jec_chargeitem_id'] ?>" 
						value="<?= $chargelist['name'] ?>" size="38" />
					</td>
					<td>
						<input type="text" id="<?= 'description'.$chargelist['jec_chargeitem_id'] ?>" 
						value="<?= $chargelist['description'] ?>" size="88" />
					</td>
					<td align="center">
						<?php if ($authority['isupdate']=='Y'): ?>
							<input type="button" id="<?= 'btn_update'.$chargelist['jec_chargeitem_id'] ?>" 
							value="修改" onclick="data_update(<?= $chargelist['jec_chargeitem_id'] ?>)" />
						<?php else: ?>
							---
						<?php endif; ?>
					</td>
					<td align="center">
						<?php if ($authority['isdelete']=='Y'): ?>
							<input type="button" id="<?= 'btn_delete'.$chargelist['jec_chargeitem_id'] ?>" 
							value="刪除" onclick="data_delete(<?= $chargelist['jec_chargeitem_id'] ?>)" />
						<?php else: ?>
							---
						<?php endif; ?>
					</td>
		...
		<script type="text/javascript">
			...
			function data_new()
			{
				if ($.trim($("#name").val()) == "")
				{
					humane.error("費用名稱不可空白");
					return;
				}
				//if (! confirm("是否確認要新增費用名稱?")) return;
				para_data = {
					name: $("#name").val(),
				...
				$.ajax({
					type: "POST",
					url: "<?=site_url('ecp_data_title/charge_new/')?>",
					data: (para_data),
					success: function(result){
						if (result)
						{
							humane.info('費用名稱新增完成');
							pagereload();
						}
						else
							humane.error('費用名稱新增作業失敗！');
					}
				});
			}
			// 修改
			function data_update(id)
			{
				...
				//if (! confirm("是否確認要更新費用名稱?")) return;
				para_data = {
					jec_chargeitem_id: id,
					name: $("#name" + id).val(),
					description: $("#description" + id).val()
				};
				$.ajax({
					type: "POST",
					url: "<?=site_url('ecp_data_title/charge_update/')?>",
					data: (para_data),
					success: function(result){
						if (result)
							humane.info('費用名稱更新完成');
						else
							humane.error('費用名稱更新作業失敗！');
					}
				});
			}
			// 刪除
			...
			{
				if (! confirm("是否確認要刪除費用名稱?")) return;
				para_data = {
					jec_chargeitem_id: id
				};
				$.ajax({
					type: "POST",
					url: "<?=site_url('ecp_data_title/charge_delete/')?>",
					data: (para_data),
					success: function(result){
						if (result)
						{
							humane.info('費用名稱刪除完成');
							pagereload();
						}
						else
							humane.error('費用名稱刪除作業失敗！');
					}
				});
			}
		</script>
