...
<div id="container">
	...
	<div class="tabpage">
		<div class="clearfix">
			<div class="tab-div-left">
				<ul id="functiontabs">
					<li><a href=# onclick="loadme(0)" class="selected">料品範本列表</a></li>
	...
	<div>
		<table class="query-div" cellspacing="1" cellpadding="3" style="margin-top: 10px;">
			<tr>
				<td colspan="4" align="left">新增料品範本</td>
			</tr>
			<tr>
			...
			<tr>
				<td colspan="4" align="center">
					<?php if ($authority['isinsert']=='Y'): ?>
						<input type="button" id="btn_new" value="確定新增料品範本" onclick="data_new()" />
					<?php else: ?>
						---
					<?php endif; ?>
		...
		<!-- ----------------------------------------------------------------------------------- -->
		<!-- 現有料品範本列表(含頁次) -->
		<div id="producttemp_list">
			<?php $this->load->view('ecp_data/ecp_data_producttemp_list', $producttemp_list); ?>
		</div>
		<script type="text/javascript">
			// 新增
			function data_new()
			{
				...
				$.ajax({
					type: "POST",
					url: "<?=site_url('ecp_data_producttemp/producttemp_new/')?>",
					data: (para_data),
					success: function(result){
						if (result)
						{
							humane.info('料品範本新增完成');
							para_data1 = {
								pagestart: "<?= $pagestart ?>",
							...
							$("#producttemp_list").load('<?= site_url('ecp_data_producttemp/reload_producttemp_list/') ?>', para_data1);
							$("#btn_new").attr("disabled", false);
							$("#name").val("");
							$("#description").val("");
						}
						else
							humane.error('料品範本新增作業失敗！');
					}
				});
			}
			// 修改
			function data_update(id)
			{
				...
				//if (! confirm("是否確認要更新料品範本?")) return;
				para_data = {
					jec_producttemp_id: id,
					name: $("#name" + id).val(),
					description: $("#description" + id).val()
				};
				$.ajax({
					type: "POST",
					url: "<?=site_url('ecp_data_producttemp/producttemp_update/')?>",
					data: (para_data),
					success: function(result){
						if (result)
							humane.info('料品範本更新完成');
						else
							humane.error('料品範本更新作業失敗！');
					}
				});
			}
			// 刪除
			...
			{
				if (! confirm("是否確認要刪除料品範本?")) return;
				para_data = {
					jec_producttemp_id: id
				};
				$.ajax({
					type: "POST",
					url: "<?=site_url('ecp_data_producttemp/producttemp_delete/')?>",
					data: (para_data),
					success: function(result){
						if (result)
						{
							humane.info('料品範本刪除完成');
							para_data1 = {
								pagestart: "<?= $pagestart ?>",
								sort_field: $("#sort_field").val(),
								sort_sequence: $("#sort_sequence").val(),
			...
			function data_sort(sort_field)
			{
				...
				$("#producttemp_list").load('<?= site_url('ecp_data_producttemp/reload_producttemp_list/') ?>', para_data);
			}
			// 編輯內容
			function data_edit(id)
			{
				location.href = "<?=base_url().'ecp_data_producttemp/producttemp_edit/' ?>" + id + "/" + $("#sort_field").val() + "/" + $("#sort_sequence").val() + "/" + "<?= $pagestart ?>";
			}
		</script>
	</div>
	</div>
