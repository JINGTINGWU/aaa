...
//$pd_pp=10;	
        ...
        switch($df_ip['ac']):
            case 'list': 
				...
				$final['task_list_url']=site_url($final['var_purl'].'task_list_index/list_div/'.$df_ip['key_id'].'/created/asc/0/N/');
				$final['file_list_url']=site_url($final['var_purl'].'task_list_index/file_list_div/'.$df_ip['key_id'].'/'.$_SESSION[$final['pg_tag']]['fob'].'/'.$_SESSION[$final['pg_tag']]['fot'].'/0/N/');
				...
				$projt_v_set=$this->CM->Init_TB_Set('mm_project_init_full_set');
                ...
                $final['main_data']=array('taskdaynotice'=>1,'taskdaydelay'=>0,'taskprocesstype'=>2,'taskconfirmtype'=>1,'taskworkweight'=>1,'jec_usersuper_id_title'=>$this->GM->GetSpecData('jec_user','name','jec_user_id',$this->GM->GetSpecData('jec_project','jec_user_id','jec_project_id',$final['projj_data']['jec_project_id']))); //修改新增後的預設值為強制
				...
				$final['fpd']=$this->GM->page_data(array('now_ot'=>$_SESSION[$final['pg_tag']]['fot'],'uri_segment'=>$df_ip['seg'],'base_url'=>base_url().$this->m_controller.'/'.$this->m_function.'/'.$df_ip['tag'].'/file_list_div/'.$df_ip['key_id'].'/'.$_SESSION[$final['pg_tag']]['fob'].'/'.$_SESSION[$final['pg_tag']]['fot'].'-'.$_SESSION[$final['pg_tag']]['fpp'].'/','mm_spec_url'=>array('ot_desc_url'=>'DESC@@@'.$_SESSION[$final['pg_tag']]['fot'],'ot_asc_url'=>'ASC@@@'.$_SESSION[$final['pg_tag']]['fot'],'ob_task_name_url'=>'task_name@@@'.$_SESSION[$final['pg_tag']]['fob'],'ob_filename_url'=>'filename@@@'.$_SESSION[$final['pg_tag']]['fob'],'ob_uploader_name_url'=>'uploader_name@@@'.$_SESSION[$final['pg_tag']]['fob'])));
				...
				$final['proj_data']=$this->GM->common_ac('list',array('info'=>$proj_v_set['mm_set'],'upt'=>'def','kid'=>$final['projj_data']['jec_project_id']));
				...
				$final['tcate_url']=array(
						'project_init_index'=>base_url($final['var_purl'].'project_init_index/list/'.$final['projj_data']['jec_project_id'].'/'),
						'project_list_index'=>base_url($final['var_purl'].'project_list_index/list/0/created/asc/0/N/'),
						'project_adjust_index'=>base_url($final['var_purl'].'project_adjust_index/list/'.$final['projj_data']['jec_project_id'].'/'),
						'project_overview_index'=>base_url($final['var_purl'].'project_overview_index/list/'.$final['projj_data']['jec_project_id'].'/'),
						'job_list_index'=>base_url($final['var_purl'].'job_list_index/list/'.$final['projj_data']['jec_project_id'].'/created/asc/0/N/'),
			...
			case 'target': 
				...
				$projt_v_set=$this->CM->Init_TB_Set('mm_project_init_full_set');
				$ip_data=array('con_jec_project_id'=>$final['projj_data']['jec_project_id'],'con_jec_projjob_id'=>$final['projj_data']['jec_projjob_id'],'con_isactive'=>'Y','gp'=>'Y','pd_pp'=>$pd_pp,'pd_np'=>$df_ip['np'],'ob_'.$df_ip['ob']=>$df_ip['ot'],'con_jec_projtask_id'=>$df_ip['task_id']);
				if($this->isadmin=='N'):
					//$ip_data['con_jec_user_id']=$this->ad_id;
				endif;
				$final['main_list']=$this->GM->common_ac('list',array('info'=>$projt_v_set['mm_set'],'type'=>'def','data'=>$ip_data));
			...
			case 'add_task'://
				...
				$gv=array("jec_task_id","description",'jec_user_id','jec_usersuper_id','startdate','enddate','price','taskname','taskprocesstype','taskdaynotice','taskdaydelay','taskworkweight','taskconfirmtype','jec_user_id_title','jec_usersuper_id_title'); $gv=$this->CM->GPV($gv);
				...
				$upd['jec_usersuper_id']=$this->$projt_set['mm_set']->get_user_id_by_title($gv['jec_usersuper_id_title']);
				unset($upd['jec_user_id_title']);
				unset($upd['jec_usersuper_id_title']);
				...
				if(substr($upd['jec_usersuper_id'],0,1)=='U'):
					$upd['jec_usersuper_id']=substr($upd['jec_usersuper_id'],2);					
				else:					
					$upd['jec_usersuper_id']=NULL;
				endif;
				$projj_v_set=$this->CM->Init_TB_Set('mm_projjob_search_set');
				$final['projj_data']=$this->GM->common_ac('list',array('info'=>$projj_v_set['mm_set'],'upt'=>'def','kid'=>$df_ip['key_id']));
				//seqno...
			...
			case 'list_div':
				...
				$pd_pp=$df_ip['pp'];
				$final['assign_view']='task_list_div';
				$this->load->library('form_input');
				$projj_v_set=$this->CM->Init_TB_Set('mm_projjob_search_set');
                ...
                $_G['pg_div_type']='';
                $_G['pg_div_id']='result_area_div'; //
				$final['pd']=$this->GM->page_data(array('now_ot'=>$df_ip['ot'],'uri_segment'=>$df_ip['seg'],'base_url'=>base_url().$this->m_controller.'/'.$this->m_function.'/'.$df_ip['tag'].'/list_div/'.$df_ip['key_id'].'/'.$df_ip['ob'].'/'.$df_ip['ot'].'-'.$pd_pp.'/','mm_spec_url'=>array('ot_desc_url'=>'DESC@@@'.$df_ip['ot'],'ot_asc_url'=>'ASC@@@'.$df_ip['ot'],'ob_seqno_url'=>'seqno@@@'.$df_ip['ob'],'ob_name_url'=>'name@@@'.$df_ip['ob'],'ob_sales_name_url'=>'sales_name@@@'.$df_ip['ob'],'ob_description_url'=>'description@@@'.$df_ip['ob'],'ob_startdate_url'=>'startdate@@@'.$df_ip['ob'],'ob_enddate_url'=>'enddate@@@'.$df_ip['ob'],'ob_taskname_url'=>'taskname@@@'.$df_ip['ob'],'ob_jec_usersuper_id_url'=>'jec_usersuper_id@@@'.$df_ip['ob'],'ob_taskdaynotice_url'=>'taskdaynotice@@@'.$df_ip['ob'],'ob_taskdaydelay_url'=>'taskdaydelay@@@'.$df_ip['ob'],'ob_taskworkweight_url'=>'taskworkweight@@@'.$df_ip['ob'],'ob_taskprocesstype_url'=>'taskprocesstype@@@'.$df_ip['ob'],'ob_taskconfirmtype_url'=>'taskconfirmtype@@@'.$df_ip['ob'],'ob_price_url'=>'price@@@'.$df_ip['ob'])));
				$final['pd']['ot']=$df_ip['ot'];
				$final['pd']['ob_css']='detail-'.strtolower($df_ip['ot']).'ending';
			...
			case 'add_task_div':
				...
				$final['ip_info']['jec_user_id']['full_selected']='Y';
				$final['ip_info']['description']['style']='width:70%';
                $final['main_data']=array('taskdaynotice'=>1,'taskdaydelay'=>0,'taskprocesstype'=>2,'taskconfirmtype'=>1,'taskworkweight'=>1,'jec_usersuper_id_title'=>$this->GM->GetSpecData('jec_user','name','jec_user_id',$this->GM->GetSpecData('jec_project','jec_user_id','jec_project_id',$final['projj_data']['jec_project_id']))); //修改新增後的預設值
				$final['ip_info']['jec_task_id']['ip_dl_exclude']=$this->CM->FormatData(array('db'=>$final['main_list'],'key'=>'jec_task_id'),'page_db','s_array');
				$final['ip_info']['jec_user_id']['ld']=$this->$projt_v_set['mm_set']->get_jec_user_ld();
				$final['main_op']=$this->form_input->each_op_trans('full',$final['ip_info'],$final['main_data']);
			...
			case 'update_projtask'://
				...
				$gv=array("projtask_id",'startdate','enddate','price','jec_user_id','jec_usersuper_id','taskname','taskprocesstype','taskdaynotice','taskdaydelay','taskworkweight','taskconfirmtype','description'); $gv=$this->CM->GPV($gv);//,"description"
