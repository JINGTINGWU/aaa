...
<?php $this->load->view('common/page_bar_div',array('pd'=>$pd)); ?>
	<table class="detail-div" cellspacing="1" cellpadding="3">
        ...
        <tr id="tr_<?=$no?>">
            ...
            <td><input type="button" value="工作" onclick="JS_Link('<?=site_url($var_purl.'task_list_index/list/'.$value['jec_projjob_id'].'/created/asc/0/-1/')?>');" class="mm_submit_button"  style="width:40px;" ></td>   
            ...
            <td><input type="button" value="刪除" onclick="AWEA_Ajax('<?=site_url($var_purl.'job_list_index/delete_job/'.$value['jec_projjob_id'].'/'.$var_surl)?>','','del','tr_<?=$no?>')" class="mm_submit_button" <?=$del_da?>  style="width:40px;" ></td>  
        </tr>        
        <?php endforeach;?>
    </table>