        ...
        switch($df_ip['ac']):
            case 'list': 
				...
				$final['ip_info']['target_db']=array(
                        'call_name'=>'DB',
                        'type'=>'select',
                        'ld'=>array(array('id'=>'A','name'=>'弓銓'),array('id'=>'B','name'=>'銓準')),
						'ld_key'=>'id',
						'ld_value'=>'name',
						'full_selected'=>'N'
					);
			...
			case 'submit_purchase'://
				...
				$final['p_user']=$this->QIM->get_user_row($this->ad_id);
				$gv=array('prod_string'); $gv=$this->CM->GPV($gv);//'prod_string',"ad005053",'ad005059','ad005055','ad005005','ad005009','ad005006','ad005010','ad005013','ad005016','ad005038','ad005037','ad005054','ad005056','ad005057','ad005060','ad005018','ad005017', 
				...
				foreach($final['prod_array'] as $vv):
					if($vv>0):
						$this->db->where('jec_projprod_id',$vv)->update('jec_projprod',array('exportcode'=>$serial,'exporttime'=>$time,'isexport'=>'Y'));
				...
				$upd=array(
						'ad005001'=>$this->CM->pdb_trans('AD005'),
						//'ad005002'=>'',//
						'ad005900'=>$this->CM->pdb_trans($final['p_user']['value']),
						'ad005901'=>$this->CM->pdb_trans(date('Y/m/d H:i:s')),
						//'ad005902'=>'',
						//'ad005903'=>'',
						'ad005904'=>$this->CM->pdb_trans($ms_p_user['deptname']),//dept
						//'ad005905'=>'',
						//'ad005003'=>'',//flow
						...
						'ad005009'=>$this->CM->pdb_trans(date('Y/m/d',strtotime($need_date))),//$gv['ad005009']-需用日期
						...
						//'ad005049'=>'',
						//'ad005050'=>'',
						'ad005051'=>$this->CM->pdb_trans('否'),//是否需填寫模(治)具檢驗記錄單：是 / 否
						//'ad005052'=>'',
						...
						//'ad005067'=>'',
						//'ad005068'=>'',
						'ad005069'=>$this->CM->pdb_trans($p_user['name']),
						//'ad005070'=>'',
						//'ad005071'=>'',
				...
				//主旨
					...
					$e_upd=array(
							...
							'FieldValue'=>$this->CM->pdb_trans(date('Y/m/d').'-'.$proj_data['name'].'-'.$final['projj_data']['name'].'-'.$final['projt_data']['name']) //$現在日期$-$專案名稱$-$任務名稱$-$工作明稱$
						);
