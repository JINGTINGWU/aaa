...
//$pd_pp=5;
        ...
        switch($df_ip['ac']):
            case 'list': 
				...
				$final['main_list']=$this->GM->common_ac('list',array('info'=>$projr_v_set['mm_set'],'type'=>'def','data'=>array('con_jec_projtask_id'=>$df_ip['key_id'],'con_isactive'=>'Y','gp'=>'Y','ob_'.$df_ip['ob']=>$df_ip['ot'],'wni'=>array('recordtype'=>'2'),'pd_np'=>$df_ip['np'],'pd_pp'=>$pd_pp)));
				...
				$final['tcate_url']=array(
						'work_analysis_index'=>base_url($final['var_purl'].'work_analysis_index/list/0/startdate/asc/0/N/')
					);
            break;
			...
			case 'list_div':
				...
				$projr_v_set=$this->CM->Init_TB_Set('mm_projrecord_search_set');
				$final['main_list']=$this->GM->common_ac('list',array('info'=>$projr_v_set['mm_set'],'type'=>'def','data'=>array('con_jec_projtask_id'=>$df_ip['key_id'],'con_isactive'=>'Y','gp'=>'Y','wni'=>array('recordtype'=>'2'),'ob_'.$df_ip['ob']=>$df_ip['ot'],'pd_np'=>$df_ip['np'],'pd_pp'=>$pd_pp)));
				$projt_v_set=$this->CM->Init_TB_Set('mm_projtask_search_set');
				$final['projt_data']=$this->GM->common_ac('list',array('info'=>$projt_v_set['mm_set'],'upt'=>'def','kid'=>$df_ip['key_id']));
