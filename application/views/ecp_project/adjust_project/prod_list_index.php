...
<tr><td>
        ...
        <div id="task_info_div" class="mm_area_div_3">
        	<?php $this->load->view($this->m_controller.'/adjust_project/task_info_div_t1',array('projt_data'=>$projt_data,'record_list_index_url'=>$tcate_url['record_list_index'])); ?>
        </div>
</td></tr>
<tr><td colspan="2">
	<?php
...
$projstatus=array(3,4,5,6);
	if(in_array($proj_data['projstatus'],$projstatus)) $o_add_da='disabled';
...
<div id="search_area_div"  class="mm_area_div_1">
	...
	if($proj_data['projstatus']==6):
   		$o_add_dp='style="display:none;"';
			  ...
			  if($o_add_dp==''):
		  			$add_dp = $this->QIM->is_project_modify($proj_data['jec_project_id'])?'':'style="display:nonel"';
		      else:
			  	    $add_dp = $o_add_dp;
			  endif;
	?>
	<table <?=$add_dp?>  style="margin:0px;"  >
		<tr>
        	<th width="100">選擇料品範本：</th><td><?=$main_op['import_producttemp']['op']?><input type="button" value="從範本匯入料品清單" onclick="PG_BK_Action('import_producttemp',{})"  style="width:130px;" <?=$add_da?> /></td>
            ...
            <?php endif;?>
            <input type="button" value="即時更新" onclick="PG_BK_Action('refresh_prepare_status',{});"  class="mm_submit_button" <?php if($proj_data['exportcode']=='' or $proj_data['isworkflow']=='Y'){ echo 'disabled'; }?>  />
            </td>
        </tr>
	</table>
    </form>
    ...
    <div id="add_prod_div">
		...
		<?php $this->load->view($this->m_controller.'/adjust_project/add_prod_div',array('main_op'=>$main_op,'form_url'=>$form_url,'proj_data'=>$proj_data)); ?>
...
<div id="result_area_div"  class="mm_area_div_2">
	<?php $this->load->view($this->m_controller.'/adjust_project/prod_list_div',array('main_list'=>$main_list,'uom_pdb'=>$uom_pdb,'pd'=>$pd,'proj_data'=>$proj_data)); ?>
</div> 
</td></tr>
		...
		case 'add_projprod':
			...
			if(msg==''){
				mvalue={
					...
					vendor_name:$("#jec_vendor_id_title").val(),
...
{ var msg='';
	switch(type){
		case 'refresh_prepare_status'://			
			data={
					jec_project_id:'<?=$proj_data['jec_project_id']?>'
				};
		...
		var info=Array();
		info[0]={id:'prodname_'+data.no,msg:'料品不可空白',type:'ne'};
			info[1]={id:'quantity_'+data.no,msg:'請輸入數量',type:'nz'};
			info[2]={id:'prod_uom_id_'+data.no,msg:'請選擇單位',type:'ne'};
			/*
			info[2]={id:'price',msg:'請輸入單價',type:'nz'};
			if(document.getElementById('spec_prod_div').style.display=='block'){
				info[3]={id:'value_spec',msg:'請輸入自訂料號',type:'ne'};
