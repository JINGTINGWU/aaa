<?php
        switch($df_ip['ac']):
            case 'list': 
				...
				//echo '<br><br><br><br><br><br><br><br><br><br><br><br>'.$df_usernew_id;
				...
				if($final['projn_data']['noticetype']==9):
					...
					//echo '<br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br>'.$final['projn_data']['jec_projreply_id'].$final['main_data']['cp_recover_time'];
				endif;
			...
			case 'cp_adjust':
				$gv=array('reply','cp_time','cp_adtime_enddate','cp_adtime_startdate'); $gv=$this->CM->GPV($gv);
				$projn_set=$this->CM->Init_TB_Set('mm_projnotice_set');
				$projr_set=$this->CM->Init_TB_Set('mm_projrecord_set');
				$final['projn_data']=$this->$projn_set['mm_set']->get_projnotice_row($df_ip['key_id']);//
				$projrp_set=$this->CM->Init_TB_Set('mm_projreply_set');
			...
			case 'cp_impossible':
				...
				switch($gv['reply']):
					...
					case 'C':
						...
						$mail_content=$this->$projn_set['mm_set']->get_mail_content($mail_pdb,$mail_data['noticetype'],$mail_data);
						//update 主管的noticetype
						$this->db->where($projn_set['mm_kid'],$df_ip['key_id'])->update($projn_set['mm_tb'],array('email_content'=>$mail_content,'noticetype'=>$projn_noticetype));//無影響
						foreach($tu_user as $tu_value):
							//-Email
							//proj_notice
						...
						//異動日期
						//$this->db->query("UPDATE ".$projt_set['mm_tb']." SET oldstartdate=startdate,oldenddate=enddate WHERE ".$projt_set['mm_kid']."='".$final['projn_data']['jec_projtask_id']."'");
						$this->db->where($projt_set['mm_kid'],$final['projn_data']['jec_projtask_id'])->update($projt_set['mm_tb'],array('isconfirm'=>'Y','projtasktype'=>4,'replystatus'=>4,'canceldate'=>date('Y-m-d H:i:s'),'noticetime'=>date('Y-m-d H:i:s'),'jec_projnotice_id'=>$t_projnotice_id));//無影響,'confirmdate'=>date('Y-m-d H:i:s')
						break;
				endswitch;
				$tu_user=$this->$projt_set['mm_set']->get_jec_user_list($projt_data);
				//$tu_user=$this->$cal_set['mm_set']->get_cal_by_projtask($projt_data['jec_projtask_id']);
			...
			case 'cp_pause':
				...
				switch($gv['reply']):
					case 'Y'://OK
						...
						//異動日期
						//$this->db->query("UPDATE ".$projt_set['mm_tb']." SET oldstartdate=startdate,oldenddate=enddate WHERE ".$projt_set['mm_kid']."='".$final['projn_data']['jec_projtask_id']."'");
						$this->db->where($projt_set['mm_kid'],$final['projn_data']['jec_projtask_id'])->update($projt_set['mm_tb'],array('isconfirm'=>'Y','projtasktype'=>5,'replystatus'=>5,'noticetime'=>date('Y-m-d H:i:s'),'jec_projnotice_id'=>$t_projnotice_id));//無影響,'confirmdate'=>date('Y-m-d H:i:s')
						break;
					case 'N'://Not OK
						//update 承辦的noticetype
						$this->db->where($projn_set['mm_kid'],$df_ip['key_id'])->update($projn_set['mm_tb'],array('noticetype'=>$this->$projn_set['mm_set']->get_noticetype($df_ip['ac'].'_'.$gv['reply'])));//無影響
						...
						$this->db->where($projt_set['mm_kid'],$final['projn_data']['jec_projtask_id'])->update($projt_set['mm_tb'],array('projtasktype'=>2,'replystatus'=>0,'isconfirm'=>'N','pendingdate'=>date('Y-m-d H:i:s'),'noticetime'=>date('Y-m-d H:i:s'),'jec_projnotice_id'=>$t_projnotice_id));//無影響,'confirmdate'=>date('Y-m-d H:i:s')
						break;
				endswitch;
				$tu_user=$this->$projt_set['mm_set']->get_jec_user_list($projt_data);
				//$tu_user=$this->$cal_set['mm_set']->get_cal_by_projtask($projt_data['jec_projtask_id']);
			...
			case 'cp_recover'://recover
				...
				switch($gv['reply']):
					case 'Y'://OK
						...
						//異動日期
						//$this->db->query("UPDATE ".$projt_set['mm_tb']." SET oldstartdate=startdate,oldenddate=enddate WHERE ".$projt_set['mm_kid']."='".$final['projn_data']['jec_projtask_id']."'");
						$this->db->where($projt_set['mm_kid'],$final['projn_data']['jec_projtask_id'])->update($projt_set['mm_tb'],array('isconfirm'=>'Y','projtasktype'=>2,'replystatus'=>0,'recoverydate'=>date('Y-m-d H:i:s'),'noticetime'=>date('Y-m-d H:i:s'),'jec_projnotice_id'=>$t_projnotice_id));//無影響,'confirmdate'=>date('Y-m-d H:i:s')
						break;
					case 'N'://Not OK
						//update 承辦的noticetype
						$this->db->where($projn_set['mm_kid'],$df_ip['key_id'])->update($projn_set['mm_tb'],array('noticetype'=>$this->$projn_set['mm_set']->get_noticetype($df_ip['ac'].'_'.$gv['reply'])));//無影響
						...
						$this->db->where($projt_set['mm_kid'],$final['projn_data']['jec_projtask_id'])->update($projt_set['mm_tb'],array('confirmdate'=>date('Y-m-d H:i:s')));//無影響
						break;
				endswitch;
