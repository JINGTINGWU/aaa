...
&nbsp; <?=$main_op['excel_type']['op']?><input type="button" value="明細資料匯出Excel" class="mm_submit_button"  onclick="pg_submit('export_excel');" />
        ...
        <tr id="tr_<?=$no?>">
           ...
           <!-- <td <?=$value['prodtype']==9?$mm_work_style:''?> id="estimcostcalc_tag_<?=$no?>"><?=(float)$value['estimcostcalc']?></td>-->
            ...
            <td <?=$value['prodtype']==9?$mm_work_style:''?>><?=$e_op['description']['op']?></td> 
            <td <?=$value['prodtype']==9?$mm_work_style:''?>><input type="button" value="修改"  onclick="PG_BK_Action('update_projprod',{'projprod_id':'<?=$value['jec_projprod_id']?>','no':'<?=$no?>'})" class="mm_submit_button_1"  <?=$up_da?> ></td> 
            <td <?=$value['prodtype']==9?$mm_work_style:''?>><input type="button" value="刪除" onclick="AWEA_Ajax('<?=site_url($var_purl.'prod_list_index/delete_projprod/'.$value['jec_projprod_id'].'/'.$var_surl)?>','','del','tr_<?=$no?>')" class="mm_submit_button_1"  <?=$del_da?>></td>  
            <script>
