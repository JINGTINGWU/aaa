...
<?php /* $this->load->view('common/page_bar_div',array('pd'=>$pd)); */ ?>
	<table class="detail-div" cellspacing="1" cellpadding="3">
        ...
        <?php
		if(file_exists('uploads/'.$this->CM->ReadFileName('download','履約備品清單-'.$proj_data['name'].'.xls'))):
			?>
        <tr>
        	<td>系統產生<a href='#' onclick="PG_BK_Action('updatexls')">重新產生履約備品清單xls檔</a></td>
        	<td><?='履約備品清單-'.$proj_data['name'].'.xls'?></td>
            <td ></td>
            <td ></td>
        </tr>		
			<?php
