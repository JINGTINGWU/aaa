<?php
			...
			$edit_1_info=$this->mm_project_init_full_set->edit_1_info;
			$edit_2_info=$this->mm_project_init_full_set->edit_2_info;
...
<?php $this->load->view('common/page_bar_div',array('pd'=>$pd)); ?>
	<table class="detail-div" cellspacing="1" cellpadding="3">
    	<tr>
    		<td colspan="18" class="mm_table2_title">工作項目列表 (說明: 明細按鈕呈現黃色者, 表示已有明細在內)</td>
    	</tr>
        <tr>
            ...
            <td class="mm_table2_title2" width="60px" id="detail-normal">完成/<br />確認</td> 
            ...
            <td class="mm_table2_title2" width="40px" id="detail-normal">紀錄</td>
        ...
        <?php 
        	...
        	<?php
			if($df_ip['task_id']==$value['jec_projtask_id'])
			$ip_info['taskname']['style']="width:90%;border:solid #03C";
			else
			...
			if(!isset($e_full_set['disabled']))://非disabled				
				$edit_1_right=$this->mm_project_init_full_set->exe_right_check('check_edit_1',$value);
				$edit_2_right=$this->mm_project_init_full_set->exe_right_check('check_edit_2',$value);
        ...
        <tr id="tr_<?=$no?>">
            ...
            <td width="200"><?=$value['jec_task_id']>0?"<span id='taskname_".$no."'>".$value['taskname']."</span>":$e_op['taskname']['op']?></td>
			...
			<td><input type="button" value="明細" onclick="JS_Link('<?=site_url($var_purl.'prod_list_index/list/'.$value['jec_projtask_id'].'/created/asc/0/-1/')?>');"  class="mm_submit_button_1"  style="width:40px;<?=$value['isprojprod']=='Y'?'background-color:#FFE97F;':''?>"></td> 
            ...
            <td>
            	<?php if((in_array($value['projtasktype'],$btn_del_ty_allow)||in_array($value['projtasktype'],$btn_del_rp_allow))&&$value['isnotice']!='Y'&&$value['projtasktype']==1):?>
            	<input type="button" value="刪除"  onclick="AWEA_Ajax('<?=site_url($var_purl.'task_list_index/delete_projtask/'.$value['jec_projtask_id'].'/'.$var_surl)?>','','del','tr_<?=$no?>')"  class="mm_submit_button_1" <?=$del_da?>  style="width:40px;" >
                <?php endif;?>
                </td> 
            <td><input type="button" value="紀錄" onclick="JS_Link('<?=site_url($var_purl.'record_list_index/list/'.$value['jec_projtask_id'].'/created/asc/0/-1/')?>');"  class="mm_submit_button_1"  style="width:40px;"></td> 
